...
def _validate_context_config(config_json: dict[str, Any] | None) -> None:
    if not config_json:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="context_rule config_json is required")
    ...
    if not isinstance(groups, dict) or not groups:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="keyword_groups is required")
    if exclusions is not None and not isinstance(exclusions, list):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="exclusion_keywords must be a list")
    if not isinstance(window_size, int) or window_size <= 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="window_size must be positive")
    if not isinstance(min_condition_count, int) or min_condition_count <= 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="min_condition_count must be positive")
    if sensitivity not in {"low", "medium", "high"}:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="sensitivity is invalid")
...
def _reject_forbidden_built_in_update(rule: FilterRule, updates: dict[str, Any]) -> None:
    ...
    if forbidden or disallowed:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"{field} is not editable")
...
async def dry_run_dashboard_filter(
    ...
    if payload.rule_id is not None:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="rule_id or draft_rule is required")
