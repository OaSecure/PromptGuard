...
const apiBaseUrl = import.meta.env.VITE_PROMPTGUARD_API_BASE_URL ?? "http://localhost:8000";
...
function renderHeader(): HTMLElement {
  ...
  const overview = el("a", "nav-button", "Dashboard");
  overview.href = "index.html#admin";
  const filters = el("a", "nav-button active", "Filters");
  filters.href = "filters.html";
  const status = el("a", "nav-button", "Status");
  status.href = "status.html";
  nav.append(overview, filters, status);
