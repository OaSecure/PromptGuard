...
- Text-file upload attempts are intercepted through DOM file input/drop preflight. Supported text files are read only in memory, original filenames are not sent, Allow/Warn attempt guarded replay, and replay failure asks the user to attach the files again.
