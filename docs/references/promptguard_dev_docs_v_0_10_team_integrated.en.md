# PromptGuard Development Documentation Set v0.10 - Team Integrated Edition
...
- This v0.10 document is the single basis for PromptGuard development contracts, implementation status, API boundaries, data ownership, and work instructions.
...
### 2.1 v0.10 Terminology And Scope Rules
...
Request values required: `prompt.text`, `prompt.input_method`, `prompt.content_length`, `context.ai_service`, `context.ai_service_domain`, `context.page_url_origin`, `context.extension_version`, `context.browser`, `context.locale`, `filter_config_version`, `client_request_id`.

Request values prohibited: `user_id`, `workspace_id`, full page URL path/query, original filename, secrets in IDs.

Response values required: `event_id`, `request_id`, `risk_score`, `risk_level`, `action`, safe `user_message`, `allow_original_send`, `requires_justification`, metadata-only `detections[]`, optional `business_context_matches[]`, machine-facing `filter_config_version`, optional `masked_prompt` only when `action=Mask`, optional `partial_result`.

Response values prohibited: raw prompt echo, raw detected value, internal stack trace, arbitrary exception text, full masked prompt persisted in event/dashboard APIs.

### 7.2 File Analyze Boundary

Endpoint: `POST /files/analyze`

MVP file scope is text-family files only. PDF, Office documents, OCR, archive extraction, malware scanning, and binary analysis are outside MVP. File content is transient input only and must not be stored, logged, or shown on the dashboard.
...
| --- | --- | --- | --- |
| API server | Python self-hosted API | `apps/api/` | FastAPI, Pydantic v2, SQLAlchemy 2.x, Alembic. Includes API schema, auth, detectors, unified filter rules, masking, event service. |
| Dashboard | Vanilla TypeScript SPA admin UI | `apps/dashboard/` | Plain HTML, CSS, and TypeScript without React, Vue, Svelte, or Next.js. Includes login, overview, events, users, filters, status screens. |
...
| Dashboard | `cd apps/dashboard && npm test` | login, overview, events, users, filters, status UI pass |
...
`Document order` is renumbered from 1 inside this v0.10 document for readability. The Owner and Planned date columns reflect the revised WBS allocation after one team member was removed. Area, category, item, original detail, status, and implementation instructions are kept intact unless needed for allocation consistency.
...
Each WBS row's `v0.10 implementation instruction` is an implementation ticket containing:
...
| Document order | Phase | Category | Item | Owner | Planned date | Area | Original detail | Current status | v0.10 implementation instruction |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Preparation/Planning | Scope confirmation | Confirm open-source MVP scope | 전체 | 2026-05-19 | Planning·QA·Docs | Self-host, admin-managed user creation, no raw storage, filter rule scope table | Partial | Remaining: align README/install/admin/privacy docs with v0.10 MVP scope. Prereq: v0.10 contract fixed. PR criterion: self-host, admin-managed user creation, no raw storage, filter rule, dashboard scope are described consistently in docs and repo structure. Verification: document grep and final demo checklist. |
...
| 5 | Preparation/Planning | Verification plan | List tests and release gates | 전체 | 5-22(Fri) | Planning·QA·Docs | Install, E2E, privacy, security tests | Partial | Remaining: connect API/dashboard/extension/privacy/security/release gates to implementation subplans and CI commands. Prereq: each app scaffold and test runner. PR criterion: every MVP slice has completion criteria and test command. Verification: `pytest`, dashboard test, extension checks, Docker smoke, privacy regression. |
| 6 | OSS·Seed·Auth | Repository | Create monorepo base structure | 김현성 | 5-22(Fri) | Chrome Extension | apps/api, apps/dashboard, apps/extension, packages, docs, infra | Partial | `apps/extension` and `apps/api` for the API core now exist. Dashboard, infra, and root runtime scripts remain next implementation targets. |
| 7 | OSS·Seed·Auth | Runtime | Docker Compose runtime configuration | 유지수 | 5-24(Sun) | Chrome Extension | API, dashboard, PostgreSQL, Redis compose file | Partial | `infra/docker-compose.yml` now contains the API+PostgreSQL default runtime and optional Redis profile. Dashboard service, PostgreSQL-backed `/readyz`, and migration smoke remain. |
...
| 9 | OSS·Seed·Auth | Build | Organize common build scripts for API, UI, extension | 김현성 | 5-23(Sat) | Chrome Extension | dev/build/test scripts and package commands | Partial | Root workspace is needed, but JS-only workspace assumptions must be revisited because the server is Python. |
| 10 | OSS·Seed·Auth | Status check | Implement server health check endpoint | 유지수 | 5-24(Sun) | Server·Security | `/healthz` response and dependency status | Partial | `/livez` is implemented. `/readyz`, `/healthz`, DB/migration dependency status, and dashboard status schema remain. |
| 11 | OSS·Seed·Auth | Migration | Build DB migration runtime skeleton | 유지수 | 5-24(Sun) | Server·Security | fresh install/restart migration verification | Not Done | Write migration runner based on SQLAlchemy 2.x + Alembic. |
| 12 | OSS·Seed·Auth | Seed/Auth | Implement default ADMIN seed readiness check | 김영은 | 5-24(Sun) | Server·Security | `seed/readiness`, needs_setup response | Not Done | Write Python API endpoint and tests. |
| 13 | OSS·Seed·Auth | Seed/Auth | Implement default ADMIN DB seed | 김영은 | 5-24(Sun) | Server·Security | create workspace, default ADMIN, default filter config | Not Done | Implement idempotent DB seed/migration and safe audit metadata. No user-facing first-admin API. |
| 14 | OSS·Seed·Auth | Seed/Auth | Implement seed idempotency and audit record | 김영은 | 5-24(Sun) | Server·Security | setup lock, SETUP_COMPLETED audit | Not Done | Implement idempotency constraint and audit metadata for seed execution. |
| 15 | OSS·Seed·Auth | Initial screen | Implement login page | 김영은 | 5-25(Mon) | Dashboard·UI | login page and ADMIN session navigation | Not Done | Implement dashboard login page, session check, logout path, and session-expired redirect. |
...
| 17 | OSS·Seed·Auth | Account DB | Create users, invites, registration_settings tables | 유지수 | 5-24(Sun) | Server·Security | users and refresh token tables; no MVP invite/registration tables unless needed later | Not Done | Write PostgreSQL migration. |
| 18 | OSS·Seed·Auth | Password | Store password hash | 유지수 | 5-24(Sun) | Server·Security | Argon2id/bcrypt, plaintext-not-stored test | Not Done | Prefer Argon2id and make cost parameters configurable in operations. |
| 19 | OSS·Seed·Auth | Login | Implement login, refresh, auth/me API | 유지수 | 5-24(Sun) | Server·Security | issue access/refresh token and return user info | Partial | `/auth/me` skeleton is implemented. Real login/refresh/logout, token verification, raw refresh-token non-storage, revocation/rotation tests remain. |
| 20 | OSS·Seed·Auth | Token protection | Handle refresh token hash, expiry, revocation | 유지수 | 5-27(Wed) | Server·Security | verify refresh_tokens raw value not stored | Not Done | Implement token hash, expiry, revoke, rotation tests. |
| 21 | OSS·Seed·Auth | Permission | Implement ADMIN/USER permission middleware | 유지수 | 5-27(Wed) | Server·Security | USER access to Admin API returns 403 | Not Done | Apply role guard and 403/404 rule. |
| 22 | OSS·Seed·Auth | Invite | Implement ADMIN user creation API | 김현성 | 5-27(Wed) | Server·Security | POST /admin/users creates USER or ADMIN; reject invalid role/input | Not Done | Implement POST /admin/users with password hashing, role validation, and safe audit metadata. |
| 23 | OSS·Seed·Auth | Invite management | Implement admin user list/detail API | 김현성 | 5-27(Wed) | Server·Security | GET /admin/users list, user aggregate fields, safe metadata only | Not Done | Implement ADMIN-only user list/detail API and safe error handling. |
...
| 25 | OSS·Seed·Auth | User management | Implement user status/role change API | 김현성 | 5-29(Fri) | Server·Security | ACTIVE/DISABLED, USER/ADMIN change | Not Done | Implement ADMIN user management API. |
| 26 | OSS·Seed·Auth | Auth verification | Write admin-managed user creation/login/permission tests | 유지수 | 5-29(Fri) | Planning·QA·Docs | auth/RBAC integration tests | Not Done | Write pytest/API integration tests. |
| 27 | OSS·Seed·Auth | Security settings | Apply CORS and base rate-limit rule | 유지수 | 5-29(Fri) | Server·Security | allowed origins, auth/analyze request limits | Not Done | Implement explicit CORS and start with in-process/Postgres rate limit. |
| 28 | Analysis/Detection | Request validation | Validate Analyze API request schema | 김현성 | 5-29(Fri) | Server·Security | validate prompt/context/filter_config_version/client_request_id | Partial | Pydantic schema, FastAPI route, OpenAPI exposure, and route tests are implemented. Real auth/workspace context and DB-backed filter config context integration remain. |
| 29 | Analysis/Detection | Raw-source protection | Implement raw_prompt non-storage boundary | 김현성 | 5-30(Sat) | Planning·QA·Docs | block request body logging and redaction hook | Partial | Safe redaction helpers, safe problem responses, and raw-prompt non-echo tests are implemented. Real access/request/error logging blocks, privacy scans, and DB/log integration tests remain. |
...
| 31 | Analysis/Detection | Hash | Implement HMAC prompt_hash | 김현성 | 5-31(Sun) | Server·Security | workspace-separated hash and secret injection | Partial | HMAC helpers that separate workspace id as input, env-based secret settings, and route-boundary generation are implemented. Real auth workspace id, key id, rotation, and event persistence integration remain. |
...
| 34 | Analysis/Detection | Personal data | Implement email/phone detection | 유지수 | 5-31(Sun) | Server·Security | EMAIL/PHONE detection functions and unit tests | Not Done | Deterministic detectors plus corpus tests. |
| 35 | Analysis/Detection | Personal data | Implement Korean RRN checksum validation | 유지수 | 6-1(Mon) | Server·Security | valid/invalid dummy RRN tests | Not Done | Dummy-only checksum tests. |
| 36 | Analysis/Detection | Personal data | Implement card number Luhn validation | 유지수 | 6-1(Mon) | Server·Security | detect only Luhn-valid numbers | Not Done | Luhn test vectors. |
...
| 44 | Analysis/Detection | Context classification | Implement contract-information rule classifier | 김현성 | 6-4(Thu) | Planning·QA·Docs | contract amount, penalty, NDA context tests | Partial | Contract amount, penalty, and NDA helpers plus unit tests are implemented. Corpus expansion and detector pipeline/scoring integration remain. |
...
| 53 | Analysis/Detection | Merge | Implement overlap merge rules for detections | 김현성 | 6-6(Sat) | Server·Security | secret priority, longer span priority tests | Partial | Helper and tests are implemented for secret priority and longer span priority within the same priority level. Integration into the full detector pipeline to avoid duplicate statistics remains. |
...
| 55 | Analysis/Detection | Masking | Replace personal data/secrets with placeholders | 유지수 | 6-7(Sun) | Server·Security | replace repeated PII/API_KEY/DB URL values | Not Done | Server-side masking with no storage. |
...
| 58 | Extension | Server connection | Implement self-host API URL input screen | 김현성 | 6-7(Sun) | Chrome Extension | API base URL storage and connection verification | Partial | Mock/real connection UI and `/auth/me` skeleton exist. Real token verification and end-to-end options smoke remain. |
| 59 | Extension | Login | Handle extension login/token storage | 김현성 | 6-7(Sun) | Chrome Extension | token storage, refresh, logout behavior | Partial | Token storage exists; real refresh/logout server integration remains. MV3 service worker inactivity is not auth expiry; attempt automatic refresh first on access-token expiry. |
| 60 | Extension | Config sync | Sync server selector and filter config | 김현성 | 6-8(Mon) | Chrome Extension | `/config/extension` call and cache | Partial | Client/cache and `/config/extension` skeleton exist. DB-backed filter config/selector source and real smoke remain. |
...
| 66 | Extension | API integration | Implement Analyze API client | 김현성 | 6-10(Wed) | Chrome Extension | request body creation, 401/timeout handling | Partial | The extension client and `/auth/me`, `/config/extension`, `/prompts/analyze` route skeletons exist. Real token verification, DB-backed filter config, and end-to-end smoke remain. |
...
| 73 | Extension | Notice/status | Storage/non-storage notice and connection status screen | 김현성 | 6-12(Fri) | Chrome Extension | notice, filter config sync time, server status | Partial | Server status endpoint does not exist yet. |
| 74 | Dashboard/Admin | Screen skeleton | Build dashboard routing and layout | 김영은 | 6-13(Sat) | Dashboard·UI | routing, auth guard, shared layout | Not Done | Implement dashboard scaffold. |
| 75 | Dashboard/Admin | Login screen | Connect login screens | 김영은 | 6-14(Sun) | Dashboard·UI | login screen and admin session flow | Not Done | Implement UI after auth API. |
...
| 77 | Dashboard/Admin | Summary | Implement overview cards and trend charts | 김영은 | 6-14(Sun) | Dashboard·UI | event, Warn, Mask, Block cards | Not Done | MVP required. Display event/user/period statistics on the first screen. |
| 78 | Dashboard/Admin | Events | Implement Risk Events list and filters | 김영은 | 6-14(Sun) | Dashboard·UI | period, type, action, risk filters | Not Done | MVP required. Implement period, user, action, risk, detector, service/domain filters without raw values. |
| 79 | Dashboard/Admin | Events | Implement raw-data-free event detail | 김영은 | 6-14(Sun) | Dashboard·UI | show event_id, type, score, detection summary, and no version identifier | Not Done | MVP required. Needs safe metadata detail, matched keyword counts, and privacy UI tests. |
| 80 | Dashboard/Admin | User stats | Implement per-user event stats API | 유지수 | 6-15(Mon) | Server·Security | per-user type/count/action distribution API | Not Done | MVP required. Separate user aggregate API from follow-up drilldown API. |
| 81 | Dashboard/Admin | User stats | Implement per-user event table | 유지수 | 6-15(Mon) | Dashboard·UI | user, department, top detection, last event | Not Done | MVP required. Top-user/user summary table; detailed user audit page is follow-up. |
...
| 83 | Dashboard/Admin | User management | Implement Users management screen | 유지수 | 6-15(Mon) | Dashboard·UI | list, role/status change | Not Done | Admin UI. |
...
| 87 | Dashboard/Admin | Filter rule | Implement filter rule management screen | 유지수 | 6-17(Wed) | Dashboard·UI | filter list, create, update, dry-run UI | Not Done | After filter rule APIs. |
| 88 | Dashboard/Admin | Status | Server health/degraded status screen | 유지수 | 6-17(Wed) | Dashboard·UI | API/DB/Redis status display | Not Done | Redis only when enabled. |
...
| 95 | Integration·Security·Docs | E2E | Write selector-change regression tests | 전체 | 6-21(Sun) | Chrome Extension | remote selector update fixture | Partial | Extension fixture exists. Real config endpoint is needed. |
...
- Implementation locations: `apps/extension/*`, `apps/api/*` modules for invites, user management, analyze/filter rule/idempotency/hash, extension API adapter, and tests.
- Prerequisites: Python API scaffold, auth context, PostgreSQL idempotency/event tables, user tables, OpenAPI output.
- Remaining implementation: real self-host API smoke, real token verification, DB-backed filter/config, raw prompt logging block, duplicate `client_request_id` handling, HMAC persistence integration, admin user management reassigned work, rule-pack/ambiguous handling, filter rule CRUD/pipeline, and overlap merge pipeline integration.
- PR completion criterion: extension DOM hook regression remains passing; real `/auth/me`, `/config/extension`, `/prompts/analyze` calls pass; admin user API paths assigned to this owner pass auth/RBAC tests; raw prompt and full masked prompt are absent from DB/log/error/dashboard.
...
- Prerequisites: dashboard scaffold, PostgreSQL migration baseline, auth API, metadata-only event table, metadata-only summary/events API, session auth guard.
- Remaining implementation: user-flow diagram, default admin seed/readiness/audit, event metadata tables and service, GitHub/AWS/JWT/PEM/DB URI/.env/entropy detection, internal-strategy context classifier, filter rule dry-run, login flow, overview cards/trend charts, events list/filter/detail.
...
Implementation scope: Python API foundation, Docker/PostgreSQL, migration, auth/RBAC, CORS/rate limit, PII/localized detectors, server-side masking, Analyze orchestrator, user statistics API, and dashboard management/status screens reassigned from the removed member.

- Related WBS rows: 4, 7, 10, 11, 17-21, 26, 27, 34-36, 48, 55, 56, 80-88.
...
- Implementation locations: `apps/api/*`, `infra/compose.yaml`, `.env.example`, Alembic migration, detector/masking/orchestrator modules, and future `apps/dashboard/*` users/filter rule/status screens.
- Prerequisites: repository scaffold, API dependency setup, PostgreSQL connection, settings loader, migration baseline, auth/session API.
- Remaining implementation: compose/runtime decision work, `/livez`, `/readyz`, `/healthz`, migration skeleton, account/auth/RBAC/token protection, CORS/rate limit, PII/localized detectors, filter rule table, masking, Analyze orchestrator, user stats API, user stats UI, users/filter rule/status screens.
- PR completion criterion: default Compose without Redis starts API/PostgreSQL; health/status/auth/detector/masking/analyze/user stats tests pass; reassigned dashboard management/status screens are metadata-only and RBAC-protected.
