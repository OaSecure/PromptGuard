...
| 83 | Dashboard/Admin | Users | Implement Users management screen | 유지수 | 6-16(Tue) | Dashboard·UI | admin UI | Partial | Completed slice: dashboard Users placeholder/navigation scaffold and backend `/admin/users` API exist. Status audit source: PR #22, PR #23, `apps/api/app/routes/admin_users.py`, `apps/dashboard/src/main.ts`. Remaining work: real Users management UI/API client, create/role/status form, empty/loading/error/RBAC states. |
...
- Prerequisites: repository scaffold, API dependency setup, PostgreSQL connection, settings loader, migration/auth baseline are partially implemented. Dashboard session and event/user stats APIs are still required.
...
- Remaining implementation: compose runtime smoke, filter rule table/API, Analyze orchestrator, user stats API, user stats UI, and real API-backed users/filter rule/status screens.
- PR completion criterion: default Compose without Redis starts API/PostgreSQL; remaining filter/analyze/user stats/dashboard management/status tests pass; reassigned dashboard management/status screens are metadata-only and RBAC-protected.
