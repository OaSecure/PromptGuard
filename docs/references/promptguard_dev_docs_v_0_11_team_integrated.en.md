# PromptGuard Development Documentation Set v0.11 - Team Integrated Edition
...
- This v0.11 document is the single basis for PromptGuard development contracts, implementation status, API boundaries, data ownership, and work instructions.
- The original WBS XLSX/CSV files remain separate source artifacts only for confirming scope, order, owner, and area.
- v0.11 preserves the existing v0.10 integrated contract and updates the Analyze input model from a single `prompt.text` field to typed input bundles that match real ChatGPT, Claude, and Gemini input constraints.
...
### 2.1 v0.11 Terminology And Scope Rules
...
The `POST /files/analyze` boundary described in v0.10 is not expanded into an independent final decision endpoint for the v0.11 MVP. It may remain as an extension compatibility or migration path, but the final file/attachment decision contract is absorbed into `/prompts/analyze inputs[]`.
...
`Document order` is renumbered from 1 inside this v0.11 document for readability. The Owner and Planned date columns reflect the revised WBS allocation after one team member was removed. Area, category, item, original detail, status, and implementation instructions are kept intact unless needed for allocation consistency.
...
Each WBS row's `v0.11 implementation instruction` is an implementation ticket containing:
...
| Document order | Phase | Category | Item | Owner | Planned date | Area | Original detail | Current status | v0.11 implementation instruction |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Preparation/Planning | Scope confirmation | Confirm open-source MVP scope | 전체 | 2026-05-19 | Planning·QA·Docs | Self-host, admin-managed user creation, no raw storage, filter rule scope table | Partial | Completed slice: self-host, admin-managed user creation, raw-storage prohibition, and unified Filter Rule scope are reflected in the v0.11 contract. Remaining work: align README/install/admin/privacy docs with v0.11 MVP scope. Prereq: v0.11 contract fixed. PR criterion: self-host, admin-managed user creation, no raw storage, filter rule, dashboard scope are described consistently in docs and repo structure. Verification: document grep and final demo checklist. |
| 2 | Preparation/Planning | Priority | Reclassify P0/P1/P2 requirements | 전체 | 5-20(Wed) | Planning·QA·Docs | Required/optional/follow-up scope table | Partial | Completed slice: the v0.11 document now acts as the MVP/follow-up classification source and contains current status per WBS row. Remaining work: sync MVP required, optional, follow-up scope table with this WBS. Prereq: API/dashboard/extension scope fixed. PR criterion: every WBS row has MVP/follow-up/optional status and completion criteria. Verification: review that out-of-scope features are not mixed into MVP done definition. |
...
| 5 | Preparation/Planning | Verification plan | List tests and release gates | 전체 | 5-22(Fri) | Planning·QA·Docs | Install, E2E, privacy, security tests | Partial | Completed slice: the v0.11 document contains privacy/security/E2E/release gate categories and a demo checklist. Remaining work: connect API/dashboard/extension/privacy/security/release gates to implementation subplans and CI commands. Prereq: each app scaffold and test runner. PR criterion: every MVP slice has completion criteria and test command. Verification: `pytest`, dashboard test, extension checks, Docker smoke, privacy regression. |
...
| 10 | OSS·Seed·Auth | Status check | Implement server health check endpoint | 유지수 | 5-24(Sun) | Server·Security | `/healthz` response and dependency status | Partial | Completed slice: `/livez` is implemented. Remaining work: `/readyz`, `/healthz`, DB/migration dependency status, dashboard-safe `/status/server`, and tests for required vs optional dependencies. |
...
| 19 | OSS·Seed·Auth | Login | Implement login, refresh, auth/me API | 유지수 | 5-24(Sun) | Server·Security | issue access/refresh token and return user info | Partial | Completed slice: `/auth/me` skeleton exists. Remaining work: real login/refresh/logout, token verification, raw refresh-token non-storage, revocation/rotation tests, and dashboard session separation. |
...
- Completed slices to preserve: extension DOM hook/hold/action handling, extension token storage, partial extension API client/config cache, narrow Analyze route/schema tests, safe redaction/problem response helpers, HMAC helper, contract-context helper, and overlap merge helper.
- Remaining implementation: typed `inputs[]` request body, real self-host API smoke, real token verification, DB-backed filter/config, raw prompt/clipboard/file logging block, duplicate `client_request_id` handling, HMAC persistence integration, admin user management reassigned work, rule-pack/ambiguous handling, filter rule CRUD/pipeline, and overlap merge pipeline integration.
