# PromptGuard 분석 파이프라인 개발 실행 계약서 v3.5.3
...
`schema_version="v3"`는 public Analyze API compatibility version이다. 문서 버전 `v3.5.3`과 독립적으로 유지되며, extension compatibility와 legacy adapter compatibility 검토 없이 변경하지 않는다.
...
v3.5.3은 PromptGuard의 최신 분석, 탐지, 마스킹, 정책 결정 계약이다. Scanner, classifier, verifier는 정책 판단에 필요한 evidence만 만든다. 최종 `AnalyzeResponse.action`은 `Policy Orchestrator`만 emit한다.
...
```text
PolicyDecisionRequest(request_id, input_id, source_metadata, scan_status, lexical_signals, scanner_evidence, ml_context_evidence, segment_signal_sets, classification_results, verification_results, admin_policy_config, pipeline_failures)
```
...
| ML context evidence with configured mask action and no span-based evidence in same input | warn or block according to strongest non-mask candidate; no mask target is created |
