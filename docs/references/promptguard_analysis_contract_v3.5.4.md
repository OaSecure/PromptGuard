# PromptGuard 분석 파이프라인 개발 실행 계약서 v3.5.4
...
`schema_version="v3"`는 public Analyze API compatibility version이다. 문서 버전 `v3.5.4`와 독립적으로 유지되며, extension compatibility와 legacy adapter compatibility 검토 없이 변경하지 않는다.

v3.5.4 변경사항:

* Dashboard에서 운영자가 일부 Policy Orchestrator fallback action을 수정할 수 있는 `Dashboard Policy Settings` 계약을 추가한다.
* 기본 정책은 v3.5.3과 호환되며, 기본값은 `context_classifier_action=warn`, `content_not_scanned_action=warn`, `parser_or_ocr_failure_action=warn`, `empty_input_action=allow`, `unsupported_mask_fallback_action=block`이다.
* 최종 action 결정권, action priority, privacy/storage 금지사항은 변경하지 않는다.
* `schema_version="v3"` public API compatibility는 변경하지 않는다.
...
| I-24 | persistent storage, logs, EventStorage에는 raw input text, file content, extracted text, OCR text, normalized text, atom text, segment text, raw detected value, raw span text, secret value, embedding vector, segment vector, exact model scores, exact model logits, raw file bytes, original file name, full `masked_prompt`, full `sanitized_forward_payload`를 저장하지 않는다. |
...
| I-31 | top-level `AnalyzeResponse.action` 값은 `allow`, `warn`, `mask`, `block` 네 개만 허용한다. |
| I-32 | action conflict priority는 기본적으로 `block > mask > warn > allow` 순서를 따른다. |
| I-33 | regex scanner, keyword scanner, built-in detector, LR classifier, RoBERTa verifier는 action candidate와 final action을 emit하지 않고 evidence만 생성한다. |
| I-34 | scanner/classifier/verifier evidence의 action 근거는 반드시 Admin Policy Config의 `configured_action`에서 온다. |
| I-35 | ML context label은 span을 생성하지 않으며 ML-only finding은 단독으로 마스킹 대상을 만들 수 없다. |
| I-36 | `mask` action은 원문 대신 sanitized payload를 downstream으로 전달한다는 의미다. |
| I-37 | `block` action은 원문과 sanitized payload 모두 downstream으로 전달하지 않는다는 의미다. |
| I-38 | `sanitized_forward_payload`는 response-only transient field이며 저장, 로그, dashboard API, admin summary에 남기지 않는다. |
| I-39 | persistent storage, logs, dashboard API, admin summary에는 raw input text, file content, extracted text, OCR text, raw detected value, raw span text, original file name, full `masked_prompt`, full `sanitized_forward_payload`, exact model logits, exact model scores를 저장하지 않는다. |

---

## 3.1 Action / Evidence / Policy Decision Contract

v3.5.4는 PromptGuard의 최신 분석, 탐지, 마스킹, 정책 결정 계약이다. Scanner, classifier, verifier는 정책 판단에 필요한 evidence만 만든다. 최종 `AnalyzeResponse.action`은 `Policy Orchestrator`만 emit한다.

Top-level action set은 다음 네 개만 허용한다.
...
기본 action conflict priority는 다음 순서다.

```text
block > mask > warn > allow
```

block-level finding과 mask-level finding이 동시에 존재하면 최종 action은 반드시 `block`이다.

### 3.1.1 Admin Policy Config

관리자는 다음 항목별 policy item을 설정할 수 있다.

* regex rule
* keyword rule
* built-in detector rule
* ML context label policy

각 policy item은 최소한 다음 필드를 가진다.

```json
{
  "policy_id": "string",
  "evidence_type": "regex | keyword | built_in_detector | ml_context_label",
  "label_or_rule_key": "string",
  "configured_action": "allow | warn | mask | block",
  "severity": "low | medium | high | critical",
  "reason_code": "string",
  "enabled": true,
  "source_scope": ["composer", "converted_paste", "file_ref", "parsed_file_block"],
  "masking_rule_id": "string | null"
}
```

`configured_action`은 관리자가 설정한 action이다. Regex scanner, keyword scanner, built-in detector, LR classifier, RoBERTa verifier는 이 action을 임의로 바꾸지 않는다.

### 3.1.1.1 Dashboard Policy Settings

Dashboard는 개별 detector/filter rule의 `configured_action`과 별개로, Policy Orchestrator가 fallback 또는 non-span evidence를 처리할 때 사용할 운영 정책을 수정할 수 있다.

Dashboard Policy Settings는 다음 항목을 가진다.

```json
{
  "context_classifier_action": "allow | warn | block",
  "content_not_scanned_action": "allow | warn | block",
  "parser_or_ocr_failure_action": "allow | warn | block",
  "empty_input_action": "allow | warn | block",
  "unsupported_mask_fallback_action": "warn | block"
}
```
...
| --- | --- | --- | --- |
| `context_classifier_action` | `warn` | `allow`, `warn`, `block` | LR/RoBERTa 문맥 evidence가 span 없이 위험 문맥만 제시할 때의 기본 action candidate |
| `content_not_scanned_action` | `warn` | `allow`, `warn`, `block` | unsupported file 또는 content unavailable 입력의 `CONTENT_NOT_SCANNED` notice action |
| `parser_or_ocr_failure_action` | `warn` | `allow`, `warn`, `block` | parser/OCR timeout, failure, encrypted, unavailable 상태가 policy에 전달될 때의 fallback action |
| `empty_input_action` | `allow` | `allow`, `warn`, `block` | 분석 가능한 입력이 없을 때의 info notice action |
| `unsupported_mask_fallback_action` | `block` | `warn`, `block` | `mask` action candidate가 있으나 적용 가능한 composer/converted paste span이 없을 때의 fallback action |

Dashboard Policy Settings API는 ADMIN dashboard session과 CSRF mutation boundary를 따른다.

```text
GET /dashboard/policy-settings
PATCH /dashboard/policy-settings
```

Policy settings 저장소에는 enum 값, enabled state, update metadata만 저장할 수 있다. raw prompt, file content, extracted text, OCR text, original file name, raw detected value, raw span text, embedding vector, exact model logits, exact model scores, full `masked_prompt`, full `sanitized_forward_payload`는 저장하거나 dashboard API로 노출하지 않는다.

Dashboard Policy Settings가 바꿀 수 없는 항목:

* 최종 action은 오직 Policy Orchestrator만 결정한다.
* action priority는 항상 `block > mask > warn > allow`다.
* scanner/classifier/verifier는 final action을 emit하지 않는다.
* ML-only evidence는 span을 발명하지 않으며 단독으로 `masked_prompt`를 만들 수 없다.
* file-derived content는 `masked_prompt`를 만들 수 없다.
* `unsupported_mask_fallback_action`은 `allow` 또는 `mask`가 될 수 없다.
* parser/OCR failure는 "안전함"의 증거가 아니다. 운영자가 action을 `allow`로 낮춰도 그 값은 운영 정책으로 기록될 뿐, parser/OCR failure 자체가 no-risk evidence로 바뀌지는 않는다.

### 3.1.2 Evidence generation

각 탐지 모듈은 final action이 아니라 evidence만 생성한다.

Regex, keyword, built-in detector evidence는 span-based evidence이며 마스킹 가능한 위치를 포함한다.
...
```json
{
  ...
  "policy_id": "string",
  "input_id": "string",
  "source": "composer | converted_paste | file_ref | parsed_file_block",
  "span_start": 0,
  "span_end": 0,
  "span_type": "string",
  "matched": true
...
ML context evidence는 span을 포함하지 않는다.
...
LR classifier는 candidate context label을 생성한다. RoBERTa verifier는 LR classifier가 생성한 candidate label만 검증한다. RoBERTa verifier는 새 label을 추가하지 않는다. ML context evidence는 문맥 위험을 나타낼 수 있지만 span을 제공하지 않으므로 단독으로 마스킹 위치를 만들지 않는다.

### 3.1.3 Policy Orchestrator decision flow

`Policy Orchestrator`는 다음 순서로 동작한다.

1. 모든 evidence를 수집한다.
2. 각 evidence의 `policy_id`로 Admin Policy Config를 조회한다.
3. `enabled=true` policy만 사용한다.
4. matched evidence의 `configured_action`을 action candidate로 수집한다.
5. action candidate가 여러 개면 `block > mask > warn > allow` 우선순위를 적용한다.
6. 가장 강한 action을 최종 `AnalyzeResponse.action`으로 emit한다.
7. 최종 action이 `mask`이면 sanitized payload를 생성한다.
8. 최종 action이 `block`이면 원문과 sanitized payload 모두 downstream으로 전달하지 않는다.

`Policy Orchestrator`는 action을 emit하는 유일한 컴포넌트다. Action의 근거는 반드시 Admin Policy Config의 `configured_action`이어야 한다.

### 3.1.4 Mask semantics

`mask` action은 탐지된 span을 관리자 masking rule에 따라 치환하고, 원문 대신 sanitized payload만 downstream으로 전달한다는 의미다.

Span-based finding이 `mask`로 설정되어 있고 최종 action이 `mask`이면 해당 span은 반드시 치환되어야 한다. ML-only finding은 span이 없으므로 단독으로 `mask` action을 실행할 수 없다. ML context policy에 `mask`를 허용하려면 같은 input 안에 span-based evidence가 있어야 한다.

### 3.1.5 Block semantics

`block` action은 원문도 sanitized payload도 downstream으로 전달하지 않는다는 의미다.

예:

```text
전화번호 regex policy -> mask
CONFIDENTIAL_BUSINESS_CONTEXT policy -> block
최종 action -> block
```

### 3.1.6 Response fields

Composer text와 converted paste text에는 `masked_prompt`를 사용할 수 있다.

File-derived content는 `masked_prompt`를 사용하지 않는다. File-derived content를 마스킹 후 전달해야 하는 경우에는 별도 transient response field인 `sanitized_forward_payload`를 사용한다.

```json
{
  "sanitized_forward_payload": {
    "input_id": "string",
    "source": "parsed_file_block",
    "content": "string"
  }
}
```

`sanitized_forward_payload`는 response-only field다. 저장, 로그, dashboard API, admin summary에 남기면 안 된다.

### 3.1.7 Storage and logging

저장 가능한 것은 metadata뿐이다.
...
* original file name
* full `masked_prompt`
* full `sanitized_forward_payload`
* exact model logits
* exact model scores
...
* failure message는 raw input text, raw file bytes, OCR text, extracted text, original file name, temp file path, rendered image path, matched value를 포함하면 안 된다.
...
* Extension local storage must not persist raw input text or file content.
...
* Route는 raw input text, file content, OCR text를 log에 남기지 않는다.
...
저장 금지: request body raw input text, uploaded file bytes, full `masked_prompt`, full `sanitized_forward_payload`, full URL path/query, original file name. 저장 허용: request_id, login_id, extension_version, schema_version, request size bucket, non-sensitive failure code.
...
저장 금지: raw input text, file content, extracted text, OCR text, parsed text, normalized text, atom text, segment text, embedding vector, raw category_scores, raw suppressor_scores, verifier input dump, raw detected value, raw span text, original file name, full `masked_prompt`, full `sanitized_forward_payload`. 저장 허용: request_id, input_id, status code, failure code, action/reason metadata after policy.
...
`Policy Orchestrator`는 parser status, scanner evidence, classifier/verifier evidence, source metadata, Admin Policy Config를 조합해 final action, recommended_action, reason_code, severity를 확정한다.
...
* Admin Policy Config 조회
* enabled policy filter 적용
* matched evidence의 `configured_action` 수집
* action conflict priority 적용
* final action 결정
...
* fallback policy 적용
* file-derived `masked_prompt` 금지 적용
* span-based mask payload 생성
* block action의 downstream 전달 차단
...
* scanner/classifier/verifier evidence에 없는 label을 생성하지 않음
* ML-only finding에서 span을 발명하지 않음
...
* Top-level action set is only `allow`, `warn`, `mask`, `block`.
* Action candidates come from matched evidence's Admin Policy Config `configured_action`.
* Regex scanner, keyword scanner, built-in detector, LR classifier, and RoBERTa verifier do not emit final action.
...
* ML context label has no span and cannot create a masking target by itself.
* If block-level and mask-level findings both exist, the final action is `block`.
* Dashboard Policy Settings can change fallback action candidates, but cannot change final action ownership, action priority, or privacy/storage rules.
...
* `masked_prompt` is limited to composer text and converted paste text.
* file-derived sanitized content, when needed for downstream forwarding, uses response-only `sanitized_forward_payload`.
* `sanitized_forward_payload` must not be stored, logged, exposed through dashboard API, or included in admin summary.
...
| parser timeout | `parser_or_ocr_failure_action`; default warn |
| unsupported file | `content_not_scanned_action`; default warn with content_not_scanned notice |
| empty input | `empty_input_action`; default allow with info notice |
| mask candidate without supported mask target | `unsupported_mask_fallback_action`; default block |
| policy conflict | higher priority action wins |
| reason code missing | internal unmapped fallback |

#### Privacy / storage rules

저장 금지: raw input text, file content, extracted text, OCR text, normalized text, segment text, raw secret, raw detected value, raw span text, original file name, full `masked_prompt`, full `sanitized_forward_payload`, exact model logits, exact model scores. 저장 허용: action/recommended_action/reason_code/severity/category and EventMetadata allowlist metadata.

#### Unit tests

* `test_policy_action_priority`
* `test_policy_timeout_is_not_allow_reason`
* `test_policy_uses_configured_action_as_action_candidate`
* `test_policy_uses_dashboard_policy_settings_for_fallback_actions`
* `test_unsupported_mask_fallback_rejects_allow_and_mask`
* `test_policy_block_overrides_mask_when_both_exist`
* `test_ml_only_mask_requires_span_based_evidence`

#### Contract tests

* `test_policy_orchestrator_only_module_emits_action`
* `test_policy_confirmed_secret_evidence_overrides_roberta_reject`
* `test_scanner_classifier_verifier_never_emit_final_action`
* `test_policy_orchestrator_is_only_analyze_response_action_owner`

#### Integration tests

* `test_file_risk_does_not_generate_masked_prompt`
* `test_response_boolean_mapping_for_block_mask_warn_allow`
* `test_mask_action_downstream_payload_has_no_raw_detected_value`
* `test_block_action_forwards_no_original_or_sanitized_payload`
...
* `mask` forwards sanitized payload only.
* `block` forwards neither original payload nor sanitized payload.
...
저장 금지: source text, file content, extracted text, OCR text, normalized text, segment text, raw file bytes, raw secret, raw detected value, raw span text, original file name, full `masked_prompt`, full `sanitized_forward_payload`, matched lexical value, worker queue dump, raw protected target text, raw protected target pattern, human-readable protected target canonical name, exact model scores, exact model logits, exact size_bytes. 저장 허용: EventRecord allowlist only.
...
* file-derived content를 마스킹 후 downstream으로 전달해야 하는 경우 response-only `sanitized_forward_payload`를 사용한다.
* `sanitized_forward_payload`는 저장, 로그, dashboard API, admin summary에 남기지 않는다.
* file_reference input에서 발견된 risk는 Admin Policy Config와 evidence에 따라 allow, warn, mask, block 중 하나의 최종 action으로 귀결될 수 있다.
* composer risk와 file risk가 함께 있으면 top-level action은 `block > mask > warn > allow` 우선순위를 따른다.
* block-level finding과 mask-level finding이 동시에 있으면 top-level action은 반드시 `block`이다.
* ML-only finding은 span이 없으므로 단독으로 마스킹 대상을 만들 수 없다.
* ML context policy에 `mask`를 허용하려면 같은 input 안에 span-based evidence가 있어야 한다.
* parser/OCR failure는 allow 근거가 아니다.
* unsupported/content unavailable attachment는 `CONTENT_NOT_SCANNED` evidence로 policy에 전달한다.
* Dashboard Policy Settings는 fallback action candidate를 바꿀 수 있지만 final action owner와 priority를 바꾸지 않는다.
* Policy Orchestrator만 final action을 결정한다.
* Policy Orchestrator는 parser/OCR status, scanner evidence, classifier/verifier evidence, source metadata, Admin Policy Config, pipeline failures를 조합해 decision을 만든다.
...
| Span-based evidence with configured mask action | mask |
| ML context evidence with configured mask action and no span-based evidence in same input | `unsupported_mask_fallback_action`; no mask target is created |
| Block candidate and mask candidate both present | block |
| Parser/OCR failure | `parser_or_ocr_failure_action`; default warn |
| Unsupported file | `content_not_scanned_action` with content_not_scanned notice; default warn |
| Empty input | `empty_input_action` with info notice; default allow |
...
* `message`에는 raw input text, extracted text, OCR text, original file name, file path, matched value를 넣지 않는다.
...
| sanitized_forward_payload | Runtime/Policy C | Policy runtime response | downstream forwarding adapter | nullable; response-only transient field | file-derived sanitized forwarding only; never EventStorage, logs, dashboard API, or admin summary |
...
* `masked_prompt` may be returned in runtime response only when composer text or converted paste text masking is required.
* `masked_prompt` must not be stored in EventStorage.
* `masked_prompt` must not be logged.
* `masked_prompt` must not be exposed through dashboard API or admin summary.
...
### 16.4 `sanitized_forward_payload` rule

* `sanitized_forward_payload` is used only when file-derived content must be masked and forwarded downstream.
* `sanitized_forward_payload` is response-only and transient.
* `sanitized_forward_payload.content` must be sanitized content, not original file-derived content.
* `sanitized_forward_payload` must not be stored in EventStorage.
* `sanitized_forward_payload` must not be logged.
* `sanitized_forward_payload` must not be exposed through dashboard API.
* `sanitized_forward_payload` must not be included in admin summary.
* A `block` final action must not include `masked_prompt` or `sanitized_forward_payload`.
...
* `test_scanner_does_not_emit_final_action`
* `test_lr_classifier_does_not_emit_action`
* `test_roberta_verifier_does_not_emit_action`
* `test_roberta_verifier_does_not_add_non_lr_candidate_label`
* `test_policy_orchestrator_only_emits_analyze_response_action`
* `test_matched_policy_configured_action_becomes_action_candidate`
* `test_policy_action_priority_block_mask_warn_allow`
* `test_block_and_mask_candidates_final_action_is_block`
...
* `test_sanitized_forward_payload_is_response_only`
* `test_event_storage_logs_dashboard_admin_summary_have_no_raw_value_or_full_masked_payload`
...
| 17 | Response and event serialization | response adapter, `masked_prompt`, `sanitized_forward_payload`, EventStorage allowlist write를 고정한다. | 14, 16 | `interfaces/http/response_adapter.py`, `privacy`, `events` | full `masked_prompt` 저장, full `sanitized_forward_payload` 저장, file-derived `masked_prompt` | response/privacy snapshot | compatibility pass | PR16 |
