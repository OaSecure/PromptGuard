...
|  | 김현성 | MVP 확장앱 | Analyze request builder | composer text, converted paste, 작은 text file, attachment metadata, unsupported attachment를 8.1의 `inputs[]` 계약으로 request body에 담는다. | request builder, input normalization, input limit preflight, API client tests | 완료 | 완료 | top-level `prompt`, `input`, `file`, `attachments` 없이 `inputs[]` 하나로 `/prompts/analyze`를 호출한다. 한도 초과 본문은 포함하지 않고 content unavailable metadata로 보낸다. | 현재 extension은 `AnalyzeRequest` builder, background analyze client, prompt/file preflight path를 unified `inputs[]` envelope로 정렬했다. request에는 `context.*`, `filter_config_revision`, `client_request_id`가 포함되고 top-level legacy `prompt`/`input`/`file`/`attachments`는 제거됐다. builder tests는 composer text, converted paste, file text, attachment metadata, unsupported attachment, limit handling, request envelope를 검증한다. |
|  | 김현성 | MVP 확장앱 | File/attachment capture | raw `File` 객체를 확보한 작은 text file은 읽어서 분석 요청에 포함하고, 그 외 첨부는 metadata-only 또는 `unsupported_attachment`로 표현한다. | file input/drop/paste handler, MIME/extension check, attachment chip metadata capture | 완료 | 완료 | 허용 text file은 `kind: "text"`, `source: "file"`, `content_included: true`로 전송된다. PDF/Office/OCR/archive/binary/image scan은 수행하지 않는다. | 현재 file input/change와 drop preflight는 허용 text file만 메모리에서 읽어 `kind: "text"`, `source: "file"` 입력으로 보낸다. 그 외 첨부는 attachment chip DOM capture와 metadata builder를 통해 `attachment_metadata` 또는 `unsupported_attachment`로 표현한다. visible label과 원본 파일명은 request에 싣지 않고, content unavailable/limit metadata도 contract-first tests로 고정되어 있다. |
...
|  | 김현성 | MVP API·데이터·Analyze | File/Text Input | 작은 text file scan과 attachment metadata/unsupported attachment 표현을 구현한다. | file text read path, MIME/extension allowlist, input limit handling, tests | 완료 | 완료 | 허용 text file은 `kind: "text"`, `source: "file"`, `content_included: true`로 분석된다. 미지원 첨부는 metadata-only 또는 `unsupported_attachment`로 처리된다. | 현재 Analyze request/response와 `event_inputs` schema는 `kind: "text" | "attachment_metadata" | "unsupported_attachment"`와 `source: "composer" | "converted_paste" | "file" | "attachment_chip"`를 사용한다. oversized/content unavailable 입력은 `content_unavailable_reason`, `limit_exceeded`, `content_scanned` metadata로 기록되고, 작은 text file scan과 attachment metadata/unsupported attachment 표현도 테스트로 고정되어 있다. |
