...
| 80 사용자별 이벤트 통계 API 구현 | 김현성 | MVP API·데이터·Analyze | Users 통계 API | 기본 통계 기간 내 사용자별 event/action aggregate를 `/dashboard/users` 응답에 포함한다. | `/dashboard/users` response row의 `event_count`, `blocked_count`, `masked_count`, `warned_count`, `last_event_at` | 부분 | 수정 | Users 화면의 사용자별 이벤트 표가 별도 mock 없이 `/dashboard/users` 응답만으로 렌더링된다. 기존 `/stats/users`, `user_id/display_name` 기준은 `/dashboard/users`, `login_id/username` 기준으로 교체한다. | 현재 `/dashboard/users` 응답 모델에는 사용자별 `event_count`, `blocked_count`, `masked_count`, `warned_count`, `last_event_at` 필드가 있다. 하지만 실제 route는 사용자 row만 조회하고 이벤트 통계를 계산하지 않아서, 통계 필드는 기본값 0에 머무는 상태다. 별도 `/stats/users` 쪽에는 사용자별 aggregate 계산 로직이 있으나, MVP 계약은 `/dashboard/users` 응답 row에 이 통계가 들어가는 것이다. 다음 작업은 `/stats/users`의 유효한 aggregate 로직을 `/dashboard/users` 응답으로 통합하거나 `/dashboard/users` query에서 직접 계산하는 것이다. |
...
| 87 Filter Rule 관리 화면 구현 | 유지수 | MVP 대시보드 | Filter Rule Management 화면 | `filters.html`을 `/dashboard/filters*` API와 연결한다. | `filters.html`, Filters TypeScript, Filter Rule list/form/dry-run UI | 부분 | 수정 | built-in/custom/context rule 목록, 생성·수정, enable/disable, dry-run이 동작한다. built-in detector는 허용 필드만 수정된다. | 현재 main에서 Filter Rule 화면은 가장 많이 구현되어 있다. `filters.html`이 있고, `src/filters.ts`가 `/dashboard/filters`, 상세 조회, 생성, 수정, enable/disable, delete, dry-run API를 호출한다. 즉 Filter Rule 관리 기능 자체는 상당 부분 동작 가능한 상태다. 아직 MVP 완료가 아닌 이유는 두 가지다. 첫째, 대시보드 최종 인증 방식이 아직 아니다. 현재는 bearer token으로 ADMIN을 확인하는 방식인데, MVP 대시보드는 session cookie와 CSRF를 써야 한다. 둘째, Filter Rule 변경 이력(`FilterRuleVersion`)은 MVP 이후 범위인데 현재 생성·수정·enable/disable·archive 흐름에 섞여 있다. 다음 작업은 dashboard session+CSRF를 적용하고, Filter Rule version history를 MVP 코드 경로에서 제거하거나 후속 범위로 분리하는 것이다. |
...
| 22 관리자 기반 사용자 생성 API 구현 / 23 사용자 role/status 변경 API 구현 | 김현성 | MVP API·데이터·Analyze | Users API | ADMIN 사용자 관리 API를 `/dashboard/users`, `POST /dashboard/users`, `PATCH /dashboard/users/{login_id}/role`, `PATCH /dashboard/users/{login_id}/status` 기준으로 구현한다. | users API routes, schemas, service, tests | 부분 | 수정 | `login_id` 기준 목록/생성/role/status 변경이 동작한다. email 필수 요구와 `/admin/users` path는 남기지 않는다. | 현재 `/dashboard/users` route는 있고, `login_id`, `username`, `password`, `department`, `role` 기반 사용자 생성과 role/status 변경 API도 있다. 응답에서 password/hash/token/session을 반환하지 않는 방향도 맞다. 아직 MVP 완료가 아닌 이유는 대시보드 session+CSRF가 적용되지 않았고, 사용자별 event aggregate가 실제 계산되지 않으며, 마지막 ACTIVE ADMIN 보호가 충분히 테스트되어 있지 않기 때문이다. 다음 작업은 aggregate query, session+CSRF 적용, 마지막 ACTIVE ADMIN 보호 테스트를 추가하는 것이다. |
...
| 6 analysis_events·event_detections 테이블 작성 / 7 원문 없는 이벤트 저장 서비스 구현 | 김현성 | MVP API·데이터·Analyze | Events API | 대시보드 이벤트 조회 API를 `/dashboard/events`, `/dashboard/events/{event_id}` 기준으로 구현한다. | events routes, list/detail schemas, event metadata queries, tests | 교체 필요 | 교체 | 목록/상세는 metadata만 반환하고 내부 식별자와 입력 본문을 반환하지 않는다. | 현재 이벤트 API는 존재하지만 path가 `/events`이고, 상세 응답에 `prompt_hash_prefix` 같은 내부 식별자가 포함된다. MVP 계약은 `/dashboard/events`와 `/dashboard/events/{event_id}`이며, 내부 hash/version/fingerprint는 반환하지 않아야 한다. 또한 현재 이벤트 schema는 input count, content unavailable count, input results, content unavailable inputs를 표현하지 못한다. 다음 작업은 path와 response shape를 dashboard 계약으로 교체하고, input-level event metadata를 추가하는 것이다. |
...
| 8 Filter Rule dry-run API 구현 / 29 custom keyword·regex·context_rule 생성·수정 API 구현 / 30 통합 Filter Rule 목록·상세 API 구현 | 김현성 | MVP API·데이터·Analyze | Filter Rule API | 통합 Filter Rule API를 `/dashboard/filters*` 기준으로 구현한다. | filter routes, schemas, `origin/kind`, `editable_fields`, dry-run | 부분 | 수정 | built-in detector는 `enabled`, `severity`, `action`만 수정 가능하고, custom keyword/regex/context rule과 dry-run이 동작한다. | 현재 `/dashboard/filters` API는 목록, 상세, 생성, 수정, enable/disable, delete, dry-run까지 상당 부분 구현되어 있다. built-in rule은 허용된 필드만 바꾸도록 막고, regex/context rule 검증도 있다. 즉 Filter Rule 관리 기능은 부분적으로 실제 동작한다. 아직 MVP 완료가 아닌 이유는 대시보드 session+CSRF가 붙지 않았고, MVP 이후 기능인 Filter Rule 변경 이력(`FilterRuleVersion`)이 현재 mutation 흐름에 섞여 있기 때문이다. 다음 작업은 dashboard session+CSRF를 적용하고, version history 코드는 제거하거나 post-MVP 모듈로 분리하는 것이다. |
