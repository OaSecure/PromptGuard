...
- File/Blob handle을 확보한 첨부 파일의 upload/temp `file_ref` 생성과 서버 Parser/OCR 경유 scan.
...
| `extension_connection` | Chrome 확장프로그램 사용자가 백엔드 API URL로 입력할 수 있는 내부망 origin 후보, 추천 후보에서 제외된 컨테이너 내부 origin, 포트포워딩/프록시 origin, API 포트 metadata |
...
- Chrome 확장프로그램 연동 안내가 있으면 정적 계정값이나 mock 값을 보여주지 않고, 서버가 반환한 내부망 origin 후보, 외부/포트포워딩 origin, API 포트 metadata와 OS별 확인 절차를 안전하게 표시한다. Docker bridge 같은 컨테이너 내부 origin은 Chrome 확장프로그램 추천 API URL로 표시하지 않는다
...
16. File/Blob handle을 확보한 첨부 파일은 upload/temp endpoint에서 opaque `file_ref`를 만들고, 최종 전송 검사 요청에는 `kind: "file_reference"` 입력으로 포함한다.
...
- 첨부 파일은 확장앱이 내용을 읽지 않고 upload/temp `file_ref`로 전환한 뒤 서버 Parser/OCR 경로에서 분석된다.
...
11. 파일 fixture를 첨부하고 upload/temp `file_ref` 기반 분석 결과가 event metadata로 남는지 확인한다.
...
|---|---|---|---|---|---|---|---|---|---|
| 59 Users 관리 화면 구현 | 김현성 | MVP 대시보드 | Users 화면 | mock 데이터를 제거하고 `/dashboard/users` 목록·생성·role/status API를 연결한다. | `users.html`, Users TypeScript, `/dashboard/users` API client 연동 | 완료 | 완료 | 사용자 목록, 사용자 생성, role 변경, status 변경, 비활성화가 실제 API로 동작한다. `/admin/users` 기준 연동은 남기지 않는다. | 현재 `users.html`과 `src/users.ts`는 `/dashboard/users` 목록 조회, 사용자 생성, `PATCH /dashboard/users/{login_id}/role`, `PATCH /dashboard/users/{login_id}/status`를 실제 API에 연결한다. mutation은 dashboard session cookie + CSRF를 사용하고, 401/403이면 `login.html`로 되돌린다. 사용자별 통계는 `/dashboard/users` 응답 row의 aggregate 필드로 렌더링한다. |
| 80 사용자별 이벤트 통계 API 구현 | 김현성 | MVP API·데이터·Analyze | Users 통계 API | 기본 통계 기간 내 사용자별 event/action aggregate를 `/dashboard/users` 응답에 포함한다. | `/dashboard/users` response row의 `event_count`, `blocked_count`, `masked_count`, `warned_count`, `last_event_at` | 완료 | 완료 | Users 화면의 사용자별 이벤트 표가 별도 mock 없이 `/dashboard/users` 응답만으로 렌더링된다. 기존 `/stats/users`, `user_id/display_name` 기준은 `/dashboard/users`, `login_id/username` 기준으로 교체한다. | `/dashboard/users` route가 최근 30일 `analysis_events` 기준으로 사용자별 `event_count`, `blocked_count`, `masked_count`, `warned_count`, `last_event_at`를 계산해 반환한다. 테스트는 mixed action aggregate, 기간 밖 이벤트 제외, zero-event safe metadata, password/hash/token/session-free 응답을 검증한다. Users 화면도 현재 이 aggregate 응답을 직접 렌더링한다. |
| 61 서버 상태 대시보드 구현 | 유지수 | MVP 대시보드 | Status 화면 | `status.html`을 `/dashboard/status`와 연결하고 typecheck/build를 통과시킨 뒤 변경분을 커밋·푸시·PR 생성한다. | `status.html`, Status TypeScript, `/dashboard/status` API client 연동, PR | 부분 | 수정 | API/PostgreSQL/Migration/Filter Rules/Last Checked가 표시된다. DB URL, secret, token, stack trace, 필터 설정 상세값, 환경변수 상세값은 표시하지 않는다. | 현재 `status.html`과 `src/status.ts`는 존재하고 `/dashboard/status` flat response를 렌더링한다. 다만 화면은 아직 공통 dashboard API/session helper 대신 직접 `fetch(..., { credentials: "include" })`를 사용하고, 서버 `/dashboard/status` route는 `require_dashboard_admin_session`이 아니라 bearer 기반 `require_admin`에 묶여 있다. 다음 작업은 status route와 화면을 dashboard session helper 기준으로 정렬하고 smoke를 보강하는 것이다. |
| 74, 76, 77 Dashboard prototype/static pages | 김현성 | MVP 대시보드 | Login 화면 | `login.html`을 dashboard session API와 연결한다. | `login.html`, Login TypeScript, `/dashboard/session/csrf`, `/dashboard/session/login`, `/dashboard/session/me` API client 연동 | 완료 | 완료 | ADMIN credential로 로그인하면 `overview.html`로 이동한다. USER credential은 dashboard session을 만들지 못한다. session 만료 시 login 화면으로 이동한다. | 현재 `login.html`과 `src/login.ts`는 `/dashboard/session/csrf`, `/dashboard/session/login`, `/dashboard/session/me`를 사용한다. 로그인 전 CSRF를 받고, 기존 session이 유효하면 `overview.html`로 이동하며, USER/DISABLED/invalid credential은 server-side로 거부된다. 과거의 `admin / 1234` 클라이언트 mock 비교와 `admin.html` redirect 설명은 더 이상 현재 main 상태가 아니다. |
| 74, 76, 77 Dashboard prototype/static pages | 김영은 | MVP 대시보드 | Overview 화면 | `overview.html`을 `/dashboard/overview`와 연결한다. | `overview.html`, Overview TypeScript, `/dashboard/overview` API client 연동 | 완료 | 완료 | Total Events, Blocked, Masked, Warned, Active Users 카드와 action/period 차트가 실제 API 값으로 렌더링된다. | 현재 `overview.html`과 `src/overview.ts`는 `/dashboard/overview`를 호출해 카드, action/risk/category summary, period bucket 차트를 렌더링한다. 401/403이면 `login.html`로 이동하고, logout은 dashboard session API를 사용한다. 남은 aggregate 정확도 이슈는 화면이 아니라 WBS 10 Overview API row의 범위다. |
| 78, 79 Dashboard Events metadata MVP | 김영은 | MVP 대시보드 | Events 목록 화면 | `events.html`을 `/dashboard/events`와 연결한다. | `events.html`, Events TypeScript, `/dashboard/events` API client 연동 | 교체 필요 | 교체 | 이벤트 목록에 시간, 사용자, 서비스, action, risk, 대표 탐지 category/type, 탐지 수, 입력 수, content unavailable 수, 상세보기 링크가 표시된다. | 서버 `/dashboard/events` 목록 API는 존재하고 ADMIN dashboard session 기준으로 동작한다. 목록 응답은 action/risk/대표 탐지/input count/content unavailable count를 metadata-only로 반환한다. 다만 `events.html`은 여전히 정적 mock이고 `src/events.ts`는 비어 있어 실제 `/dashboard/events` 응답을 렌더링하지 않는다. 다음 작업은 mock 데이터를 제거하고 목록 API를 화면에 연결하는 것이다. |
| 78, 79 Dashboard Events metadata MVP | 김영은 | MVP 대시보드 | Event Detail 화면 | `event-detail.html?event_id=...`에서 `GET /dashboard/events/{event_id}`를 호출해 상세 metadata를 렌더링한다. | `event-detail.html`, Event Detail TypeScript, `/dashboard/events/{event_id}` API client 연동 | 교체 필요 | 교체 | detection summary, detections, input results, content unavailable inputs, Business Context metadata가 표시된다. 입력 본문, 파일 내용, 전체 `masked_prompt`, 원본 파일명, 내부 식별자는 표시하지 않는다. | 서버 `/dashboard/events/{event_id}` detail API는 존재하고 dashboard session 기준으로 동작하며, detection summary, detections, input results, content unavailable inputs, Business Context metadata를 metadata-only로 반환한다. 서버 응답은 prompt hash 계열 내부 식별자를 반환하지 않는다. 남은 문제는 `event-detail.html`과 `src/event-detail.ts`가 아직 `event_id` 기반 API 호출이 아닌 정적 mock 게시판이고, 정적 화면 안에 "프롬프트 해시" 같은 내부 식별자 표시가 남아 있다는 점이다. |
| 87 Filter Rule 관리 화면 구현 | 유지수 | MVP 대시보드 | Filter Rule Management 화면 | `filters.html`을 `/dashboard/filters*` API와 연결한다. | `filters.html`, Filters TypeScript, Filter Rule list/form/dry-run UI | 완료 | 완료 | built-in/custom/context rule 목록, 생성·수정, enable/disable, dry-run이 동작한다. built-in detector는 허용 필드만 수정된다. | `filters.html`과 `src/filters.ts`는 `/dashboard/filters`, 상세 조회, 생성, 수정, enable/disable, delete, dry-run API를 사용한다. 대시보드 경로는 ADMIN session cookie + CSRF mutation 경계로 동작하고, built-in detector는 허용 필드만 수정 가능하다. Filter Rule 변경 이력은 MVP 이후 범위로 두고 현재 화면 경로에서는 생성·수정·enable/disable·archive side effect를 만들지 않는다. |
| 74, 76, 77 Dashboard prototype/static pages | 김현성 | MVP 대시보드 | Dashboard 공통 API client | dashboard 화면들이 ADMIN session, CSRF, safe error banner, loading/empty state를 공통 방식으로 처리한다. | dashboard shared API client, session helper, CSRF helper, error rendering helper | 부분 | 수정 | `login.html`, `overview.html`, `events.html`, `event-detail.html`, `users.html`, `filters.html`, `status.html`에서 공통 client가 재사용된다. session id는 `localStorage`에 저장하지 않는다. | 현재 `src/dashboardApi.ts`와 `src/session.ts`가 존재하고 `login.ts`, `overview.ts`, `users.ts`에서 재사용된다. session id는 `localStorage`에 저장하지 않고 cookie + CSRF helper를 사용한다. 다만 모든 화면이 이 공통 경로를 쓰는 것은 아니다. `events.ts`와 `event-detail.ts`는 아직 API-backed 구현이 아니고, `status.ts`는 공통 helper 대신 직접 `fetch`를 사용한다. 다음 작업은 남은 화면도 동일한 session/CSRF/error/loading 패턴으로 정리하는 것이다. |
|  | 김현성 | MVP 확장앱 | Options auth/config | Extension options에서 PromptGuard API URL 저장, login/logout, `/auth/me`, `/config/extension` 확인을 구현한다. | options UI, service worker auth client, config cache, auth status UI | 부분 | 수정 | PromptGuard API URL 저장 후 인증 상태와 원격 확장앱 설정을 확인할 수 있다. MV3 service worker inactive 상태를 인증 만료로 표시하지 않는다. | 현재 options 화면과 background router는 PromptGuard API URL 저장, `AUTH_LOGIN_REQUEST`, `AUTH_ME_REQUEST`, `CONFIG_SYNC_REQUEST`, `/config/extension` 동기화 흐름을 갖고 있다. 서버 Analyze `inputs[]` 계약 부재가 이 row의 blocker였던 상태는 지났다. 남은 작업은 token 만료·refresh 실패·MV3 inactive 상태 구분과 real API smoke를 현재 구현 기준으로 확인하는 것이다. |
| 95 DOM selector rerender regression | 김현성 | MVP 확장앱 | Content script selector/hook | 지원 대상 AI 서비스 화면에서 composer, send button, attachment chip 후보를 감지하고 click/Enter 전송을 분석 완료 전 보류한다. | content script selector, send interceptor, replay guard, selector tests | 부분 | 수정 | `@` mention, IME composition, Shift+Enter, picker 동작을 전송으로 오판하지 않는다. protected replay는 1회만 수행한다. | 현재 content script는 composer, send button, file input/drop zone, attachment chip selector를 사용하고 click/Enter 전송을 preflight에서 보류한다. prompt/file preflight tests는 Shift+Enter, IME composition, malformed response fail-closed, 1회 replay guard를 검증한다. 남은 작업은 구현 부재가 아니라 real-service browser-safe smoke와 selector drift 대응을 현재 기준으로 확인하는 것이다. |
|  | 김현성 | MVP 확장앱 | Analyze request builder | composer text, converted paste, upload/temp `file_reference`, attachment metadata, unsupported attachment를 8.1의 `inputs[]` 계약으로 request body에 담는다. | request builder, input normalization, input limit preflight, API client tests | 완료 | 완료 | top-level `prompt`, `input`, `file`, `attachments` 없이 `inputs[]` 하나로 `/prompts/analyze`를 호출한다. file bytes, base64 payload, extracted/OCR text, original filename은 Analyze JSON에 포함하지 않는다. | 현재 extension은 `AnalyzeRequest` builder, background analyze client, prompt/file preflight path를 unified `inputs[]` envelope로 정렬했다. request에는 `context.*`, `filter_config_revision`, `client_request_id`가 포함되고 top-level legacy `prompt`/`input`/`file`/`attachments`는 제거됐다. builder tests는 composer text, converted paste, file_reference, attachment metadata, unsupported attachment, limit handling, request envelope를 검증한다. |
|  | 김현성 | MVP 확장앱 | File/attachment capture | raw `File`/`Blob` handle을 확보한 첨부는 upload/temp endpoint로 보내 opaque `file_ref`를 만들고, 최종 전송 검사 요청에는 `file_reference`로 포함한다. | file input/drop/paste handler, MIME/extension check, temp upload client, attachment chip metadata capture | 완료 | 완료 | 확장앱은 파일 내부를 읽거나 OCR/text 추출을 수행하지 않는다. 지원 파일은 `kind: "file_reference"`, 미지원/미확보 첨부는 metadata-only 또는 `unsupported_attachment`로 전송된다. | 현재 file input/change와 drop hook은 native attachment를 막지 않고 File/Blob handle을 관찰해 upload/temp `file_ref`를 등록한다. 최종 send preflight가 composer text와 등록된 `file_reference`를 함께 `/prompts/analyze`로 보낸다. visible label과 원본 파일명은 request에 싣지 않고, content unavailable/limit metadata도 contract-first tests로 고정되어 있다. |
|  | 김현성 | MVP 확장앱 | Action UX | 서버 응답의 top-level `action`, `allow_original_send`, `requires_user_confirmation`, `masked_prompt`를 기준으로 Allow/Warn/Mask/Block UX를 적용한다. | action panel, warning/confirmation UI, mask apply logic, block UI, UX tests | 완료 | 완료 | Allow는 원래 전송을 1회 재실행하고, Warn은 사용자 확인 후 전송하며, Mask는 composer text를 `masked_prompt`로 교체한 뒤 전송하고, Block은 원문 전송을 발생시키지 않는다. | 현재 prompt/file preflight controller는 top-level `action`, `allow_original_send`, `requires_user_confirmation`, `masked_prompt`만 기준으로 동작한다. Warn은 허용 플래그가 있을 때만 replay하고, Mask는 masked prompt 적용 후 replay 또는 추가 확인 후 replay하며, malformed response·timeout·network failure는 fail-closed로 처리한다. response validator는 `input_results[]`, `content_unavailable_inputs[]`, `client_request_id`, `filter_config_revision`까지 최종 계약 기준으로 검증한다. |
...
| 47 로그인·refresh·auth/me API 구현 | 유지수 | MVP 인증·세션·권한 | Auth API | 확장앱 bearer token 흐름을 `/auth/login`, `/auth/refresh`, `/auth/logout`, `/auth/me` 기준으로 구현한다. | FastAPI auth routes, token hash 저장, refresh rotation, auth tests | 부분 | 부분 | access token/refresh token 흐름이 동작하고, refresh token 원문은 저장하지 않는다. `DISABLED` 사용자는 차단된다. | 현재 `/auth/login`, `/auth/me`, `/auth/refresh`, `/auth/logout`, `/auth/change-password`와 bearer token 기반 사용자 확인은 구현되어 있다. `DISABLED` 사용자 차단과 refresh token rotation도 동작한다. Dashboard session은 별도 row에서 이미 구현되어 있으므로, 이 row를 부분으로 남기는 실제 이유는 extension bearer-token 쪽 refresh idle timeout, logout ownership 검증, refresh hardening, real-flow regression coverage가 아직 남아 있기 때문이다. |
|  | 김현성 | MVP 인증·세션·권한 | Dashboard Session API | ADMIN dashboard session을 `/dashboard/session/csrf`, `/dashboard/session/login`, `/dashboard/session/logout`, `/dashboard/session/me` 기준으로 구현한다. | dashboard session routes, CSRF, HttpOnly cookie, session hash 저장 | 완료 | 완료 | ADMIN만 session 생성 가능하고 USER는 403이다. session id는 `localStorage`에 저장하지 않는다. | `/dashboard/session/csrf`, `/dashboard/session/login`, `/dashboard/session/me`, `/dashboard/session/logout` route와 `DashboardSession` model이 구현되어 있다. session token은 hash-only로 저장되고 dashboard session cookie는 HttpOnly로 설정된다. ADMIN만 session 생성 가능하고 USER/DISABLED/invalid credential은 거부된다. mutation dependency는 dashboard session + CSRF를 요구한다. session id는 `localStorage`에 저장하지 않는다. |
| 22 관리자 기반 사용자 생성 API 구현 / 23 사용자 role/status 변경 API 구현 | 김현성 | MVP API·데이터·Analyze | Users API | ADMIN 사용자 관리 API를 `/dashboard/users`, `POST /dashboard/users`, `PATCH /dashboard/users/{login_id}/role`, `PATCH /dashboard/users/{login_id}/status` 기준으로 구현한다. | users API routes, schemas, service, tests | 완료 | 완료 | `login_id` 기준 목록/생성/role/status 변경이 동작한다. email 필수 요구와 `/admin/users` path는 남기지 않는다. | `/dashboard/users` 목록/생성/role/status API는 dashboard session/CSRF boundary를 사용하고, 응답에서 password/hash/token/session을 반환하지 않는다. 사용자별 aggregate query와 마지막 ACTIVE ADMIN 보호 테스트가 추가되어 Users API completion 기준을 만족한다. 이 API는 현재 `users.html` 화면에도 연결되어 있다. |
| 25 Analyze API 요청 schema 검증 / 57 Analyze API 전체 흐름 통합 | 김현성 | MVP API·데이터·Analyze | Analyze API | `POST /prompts/analyze`를 `inputs[]` 기반 send attempt decision endpoint로 교체한다. | analyze request/response schemas, route, pipeline orchestration, tests | 완료 | 완료 | top-level `prompt`, `input`, `file`, `attachments` 없이 `inputs[]`만 수용하고 top-level `action` 하나를 반환한다. | 현재 `/prompts/analyze`는 `inputs[]`, `context.*`, `filter_config_revision`, `client_request_id`를 받는 send-attempt decision endpoint다. response는 top-level `action`, `allow_original_send`, `requires_user_confirmation`, `detections`, `input_results`, `content_unavailable_inputs`, `business_context_matches`, `masked_prompt`를 반환한다. legacy top-level `prompt`/`input`/`file`/`attachments` request shape는 테스트로 거부된다. |
| 27 client_request_id 중복 요청 처리 | 김현성 | MVP API·데이터·Analyze | Analyze Idempotency | `(login_id, client_request_id)` 기준 중복 event 방지를 구현한다. | `idempotency_keys` 저장, idempotency service, duplicate request tests | 완료 | 완료 | 같은 전송 시도의 중복 요청이 두 번째 event를 만들지 않는다. HMAC fingerprint는 구현하지 않는다. | 현재 `idempotency_keys` metadata와 `(login_id, client_request_id)` 기준 중복 event 방지 구현이 존재한다. duplicate request는 409 duplicate response로 변환되고, 같은 사용자의 같은 `client_request_id`가 두 번째 event를 만들지 않는 테스트가 있다. HMAC fingerprint는 여전히 MVP 이후 범위다. |
|  | 김현성 | MVP API·데이터·Analyze | File/Text Input | `file_reference` scan과 attachment metadata/unsupported attachment 표현을 구현한다. | upload/temp resolver, parser/OCR worker route, MIME/extension allowlist, input limit handling, tests | 완료 | 완료 | 첨부 파일은 `kind: "file_reference"`로 분석되고, 미지원 첨부는 metadata-only 또는 `unsupported_attachment`로 처리된다. | 현재 Analyze request/response와 `event_inputs` schema는 `kind: "text" | "file_reference" | "attachment_metadata" | "unsupported_attachment"`와 `source: "composer" | "converted_paste" | "attached_file" | "attachment_chip"`를 사용한다. oversized/content unavailable 입력은 `content_unavailable_reason`, `limit_exceeded`, `content_scanned` metadata로 기록되고, file_reference와 attachment metadata/unsupported attachment 표현도 테스트로 고정되어 있다. |
| 6 analysis_events·event_detections 테이블 작성 / 7 원문 없는 이벤트 저장 서비스 구현 | 김현성 | MVP API·데이터·Analyze | Events API | 대시보드 이벤트 조회 API를 `/dashboard/events`, `/dashboard/events/{event_id}` 기준으로 구현한다. | events routes, list/detail schemas, event metadata queries, tests | 완료 | 완료 | 목록/상세는 metadata만 반환하고 내부 식별자와 입력 본문을 반환하지 않는다. | `/dashboard/events`와 `/dashboard/events/{event_id}`는 ADMIN dashboard session 기준으로 동작한다. 목록은 action/risk/대표 탐지/input count/content unavailable count를 반환하고, 상세는 detection summary, detections, input results, content unavailable inputs, Business Context metadata를 metadata-only로 반환한다. 응답은 입력 본문, 파일 내용, 전체 `masked_prompt`, 원본 파일명, prompt hash 계열, filter rule set version 같은 내부 식별자를 반환하지 않는다. |
| 10 Overview summary API 연결 | 김영은 | MVP API·데이터·Analyze | Overview API | Overview aggregate를 `/dashboard/overview` 기준으로 구현한다. | overview route, 30일 기본 aggregate, action/period counts, tests | 부분 | 부분 | 카드와 action/period 차트에 필요한 metadata가 반환된다. 사용자별 aggregate는 `/dashboard/users`가 담당한다. | 현재 `/dashboard/overview` route는 dashboard session 기준으로 존재하고, 최근 30일 event/action/risk/category/period aggregate를 반환한다. `overview.html`도 이 응답을 렌더링한다. 다만 `content_unavailable_event_count`는 아직 input-level aggregate가 아니라 0으로 고정되어 있으므로, 남은 작업은 이 필드를 현재 계약 수준까지 마무리하는 것이다. |
| 8 Filter Rule dry-run API 구현 / 29 custom keyword·regex·context_rule 생성·수정 API 구현 / 30 통합 Filter Rule 목록·상세 API 구현 | 김현성 | MVP API·데이터·Analyze | Filter Rule API | 통합 Filter Rule API를 `/dashboard/filters*` 기준으로 구현한다. | filter routes, schemas, `origin/kind`, `editable_fields`, dry-run | 완료 | 완료 | built-in detector는 `enabled`, `severity`, `action`만 수정 가능하고, custom keyword/regex/context rule과 dry-run이 동작한다. | `/dashboard/filters` API는 목록, 상세, 생성, 수정, enable/disable, delete, dry-run을 제공한다. read는 ADMIN session 기준으로, mutation과 dry-run은 ADMIN session + CSRF 기준으로 동작한다. built-in rule은 허용된 필드만 수정 가능하고 regex/context rule 검증도 있다. Filter Rule 변경 이력은 MVP 이후 범위로 두며, 현재 `/dashboard/filters*` mutation 경로는 `FilterRuleVersion` side effect를 만들지 않는다. |
| 51 서버 health/readiness endpoint 구현 | 유지수 | MVP API·데이터·Analyze | Dashboard Status API | 서버 상태 조회 API를 `/dashboard/status` 기준으로 구현한다. | status route, required/optional status metadata, tests | 부분 | 수정 | API/PostgreSQL/Migration/Filter Rules/Last Checked를 반환한다. secret, DB 연결 문자열, stack trace는 반환하지 않는다. | 현재 `/dashboard/status` route는 존재하고 flat `status`, `api_status`, `postgres_status`, `migration_status`, `filter_rules_status`, `last_checked`를 반환한다. 관련 테스트도 safe allowlist response와 비노출 항목을 검증한다. 남은 문제는 이 route가 아직 dashboard session dependency가 아니라 bearer 기반 `require_admin`을 사용한다는 점이다. |
| 6 analysis_events·event_detections 테이블 작성 | 유지수 | MVP PostgreSQL | Event schema | `analysis_events`, `event_inputs`, `event_detections` 중심의 event metadata schema를 MVP 계약에 맞게 정리한다. | SQLAlchemy models, Alembic migration, event metadata tests | 부분 | 부분 | 이벤트 metadata가 저장되고 입력 본문, 파일 내용, 전체 `masked_prompt`, 탐지값 원문은 저장되지 않는다. | 현재 `event_inputs` 기반 input-level metadata와 `event_detections` 연결이 존재하고 `/dashboard/events*` API도 `input_results`, `content_unavailable_inputs`, detection metadata를 반환한다. 과거의 `event_inputs` 부재 설명은 더 이상 현재 main 상태가 아니다. 다만 `analysis_events`에는 `prompt_hash`, `prompt_hash_key_id`, `filter_rule_set_version` 같은 compatibility residue가 남아 있어 schema cleanup은 아직 일부 남아 있다. |
| 45 users·refresh_tokens 테이블 작성 | 유지수 | MVP PostgreSQL | Auth/User schema | `users`, `refresh_tokens`, `dashboard_sessions` schema를 MVP 인증 계약에 맞게 정리한다. | SQLAlchemy models, Alembic migration, default ADMIN seed | 부분 | 부분 | `login_id`, `username`, `department`, role/status, password_hash, refresh token hash, dashboard session hash가 저장된다. | 현재 `users`, `refresh_tokens`, `dashboard_sessions` model이 존재하고, user role/status는 MVP 기준인 `ADMIN/USER`, `ACTIVE/DISABLED`로 제한된다. dashboard session hash와 csrf hash 저장 구조도 있다. 이 row를 부분으로 남기는 실제 이유는 fresh-install migration/seed smoke와 default ADMIN provisioning, auth hardening verification 같은 남은 검증 항목이지, `dashboard_sessions` 부재나 `PENDING` 상태 잔존이 아니다. |
| 48 DB migration 실행 골격 작성 | 유지수 | MVP PostgreSQL | Migration 실행 | fresh DB와 restart에서 Alembic migration과 초기 데이터 생성이 안정적으로 실행되게 한다. | Alembic env, migration runner, seed path, Docker smoke | 부분 | 수정 | fresh DB에서 migration 후 기본 ADMIN과 MVP tables가 준비된다. | 현재 main에는 `dashboard_sessions`, `event_inputs`, `idempotency_keys` 등 핵심 MVP 테이블과 seed/startup 경로가 존재한다. 남은 것은 fresh DB에서 Alembic upgrade, 기본 ADMIN seed, restart idempotency, health/readiness, dashboard login까지 이어지는 smoke 결과를 수용 게이트로 문서화하는 일이다. |
| 58 filter_rules·filter_rule_versions 테이블 작성 | 유지수 | MVP PostgreSQL | Filter Rule schema | `filter_rules` schema를 MVP 통합 Filter Rule 계약에 맞게 정리한다. | `filter_rules` model/migration, origin/kind/editable_fields/config_json columns | 부분 | 부분 | built-in detector, custom keyword, custom regex, context rule이 하나의 `filter_rules` 모델로 저장된다. | 현재 `filter_rules`는 `origin`, `kind`, `label`, `editable_fields`, `config_json` 등 MVP 계약의 핵심 필드에 가깝고, built-in/custom과 detector/keyword/regex/context_rule 구분도 있다. `/dashboard/filters*` mutation 경로는 더 이상 Filter Rule 변경 이력을 기록하지 않는다. 다만 `version` 필드와 `filter_rule_versions` table 자체는 compatibility residue로 남아 있어 schema 차원의 post-MVP history 분리 작업은 아직 남아 있다. |
...
|  | 전체 | MVP 수용·검증 | API unit/integration test | Auth, dashboard session, users, analyze, filters, events, overview, status API의 MVP 계약을 테스트한다. | API test suite, fixtures, CI command | 부분 | 수정 | `cd apps/api && pytest` 또는 대응 명령이 MVP API 계약 기준으로 통과한다. | 현재 API 테스트에는 dashboard session, users, analyze `inputs[]`, idempotency, filters, events, overview, status가 이미 포함되어 있다. 따라서 과거의 "핵심 endpoint 자체가 아직 없다"는 설명은 현재 main과 맞지 않는다. 남은 gap은 status auth boundary를 dashboard session 기준으로 정렬하는 검증, overview의 `content_unavailable_event_count`, browser-level dashboard integration, privacy/final smoke 같은 최종 수용 계층이다. |
|  | 김현성 | MVP 수용·검증 | Dashboard typecheck/build smoke | dashboard TypeScript typecheck/build와 주요 화면 loading/empty/error state smoke를 검증한다. | `npm run typecheck`, `npm run build`, dashboard smoke checklist | 부분 | 수정 | `login.html`, `overview.html`, `events.html`, `event-detail.html`, `users.html`, `filters.html`, `status.html`가 API-backed 화면으로 렌더링된다. | dashboard package에는 `login.html`, `overview.html`, `users.html`, `filters.html`, `status.html` 같은 API-backed entry와 typecheck/build 흐름이 있다. 다만 `events.html`과 `event-detail.html`은 아직 정적 mock이고, `status.ts`도 공통 session helper와 완전히 일치하지 않는다. 남은 작업은 각 화면의 loading/empty/error state smoke와 미연동 화면 정리다. |
|  | 김현성 | MVP 수용·검증 | Extension checks | selector, send hook, action UX, auth refresh, API client fixture를 검증한다. | extension checks script, fixture, manual real-service smoke | 부분 | 수정 | `python apps/extension/tests/run_extension_checks.py all` 또는 대응 명령이 통과하고 silent allow가 발생하지 않는다. | 현재 extension에는 typecheck/test/build와 `python apps/extension/tests/run_extension_checks.py all` 경로가 있고, unified `inputs[]` request, auth/config sync, prompt/file preflight, Allow/Warn/Mask/Block contract tests가 포함되어 있다. 이 row를 부분으로 남기는 이유는 서버 contract 부재가 아니라 real API/browser-safe manual smoke와 최종 수용 기록이 아직 별도 게이트로 남아 있기 때문이다. |
|  | 전체 | MVP 수용·검증 | Basic privacy smoke | 입력 본문, 파일 내용, 전체 `masked_prompt`, 탐지값 원문, 원본 파일명, dry-run sample이 저장·표시되지 않는지 확인한다. | privacy smoke test 또는 checklist | 부분 | 수정 | DB, dashboard API/DOM, error response에서 금지값이 확인되지 않는다. full privacy regression은 MVP 이후 범위다. | 설계와 README 기준의 비저장 원칙은 API/extension 구현과 여러 테스트에 반영되어 있고, `/dashboard/events*` 서버 응답도 prompt hash 계열을 반환하지 않는다. 다만 정적 `event-detail.html` mock에는 여전히 "프롬프트 해시" 표시가 남아 있고, DB/API/DOM/error response 전 경로를 한 번에 스캔하는 basic privacy smoke는 아직 별도 게이트로 확인되지 않았다. |
|  | 전체 | MVP 수용·검증 | 최종 smoke 시나리오 | 15.5의 최종 smoke 시나리오를 실행한다. | final smoke checklist, 결과 기록 | 미구현 | 신규 | fresh install부터 extension analyze, Allow/Warn/Mask/Block, dashboard metadata 확인까지 끊기지 않고 통과한다. | fresh install부터 기본 admin login, dashboard session, 사용자 생성, extension real analyze, Allow/Warn/Mask/Block, event metadata 확인까지 이어지는 MVP 전체 흐름은 아직 수용 결과로 기록되지 않았다. 과거처럼 핵심 계약 자체가 비어 있는 상태는 아니지만, fresh-install, dashboard 화면, extension real API, privacy 확인을 한 번에 묶은 최종 smoke 실행 기록이 아직 남아 있지 않다. |
...
|  |  | MVP 이후 탐지·Filter Rule·파일 분석 | File analysis | 고급 PDF/Office/OCR/archive/binary/image content scan 확장 | parser adapters, size limits, error/privacy tests | 부분 | 신규 | parser 실패 시 입력 본문/파일 내용이 error detail/log에 남지 않고 각 parser별 크기 제한이 적용된다. | 현재 기본 upload/temp `file_ref` 및 Parser/OCR 경로는 MVP 계약에 포함된다. 고급 어댑터 확장, archive/binary 심화 처리, 서비스별 첨부 고도화는 MVP 이후 범위다. |
