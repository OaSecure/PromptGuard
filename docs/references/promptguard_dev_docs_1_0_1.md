...
| Users API | 부분 | `/dashboard/users` 목록/생성/role/status patch가 있고 request/response가 `login_id`, `username`, metadata-only 형태에 맞춰져 있다. | event aggregate를 실제 `analysis_events`에서 계산하고, dashboard session + CSRF로 auth boundary를 전환하며, 마지막 ACTIVE ADMIN 보호를 계속 검증한다. |
| Dashboard Status API | 교체 필요 | 현재 구현은 `/status/server`이며 response가 `status`, `api`, `postgres`, `migrations` nested 형태다. | `/dashboard/status`로 교체하고 `api_status`, `postgres_status`, `migration_status`, `filter_rules_status`, `last_checked` flat safe metadata만 반환한다. |
| Analyze API | 교체 필요 | `/prompts/analyze`는 top-level `prompt`와 UUID typed `client_request_id`를 받고, `inputs[]` Analyze Input Bundle을 구현하지 않는다. | `inputs[]` request/response, content unavailable handling, one top-level final action, MVP idempotency model로 교체한다. |
| Event metadata schema/API | 교체 필요 | `analysis_events`, `event_detections`는 있으나 `event_inputs`가 없고 detection이 `input_id/input_index/kind/source`와 연결되지 않는다. 현재 event detail은 `prompt_hash_prefix`를 반환한다. | `event_inputs`, input-level metadata, string/enum `limit_exceeded`, `content_unavailable_reason`, `decision_basis`를 추가하고 내부 hash/revision 식별자 노출을 제거한다. |
| Idempotency | 미구현 | `idempotency_keys` table/service가 없고 `(login_id, client_request_id)` 기준 중복 event 방지가 구현되어 있지 않다. | MVP용 `idempotency_keys` 또는 동등 모델을 구현한다. HMAC request fingerprint는 post-MVP로 둔다. |
| Filter Rule model/API | 부분 | `origin`, `kind`, `category`, `label`, `severity`, `action`, `enabled`, `editable_fields`, `config_json` 방향과 `/dashboard/filters*` API가 있다. built-in field restriction도 있다. | `filter_rule_versions`를 MVP 완료 근거에서 제외하거나 post-MVP artifact로 정리하고, dashboard session + CSRF로 auth boundary를 전환한다. |
| Dashboard UI | 부분 | `admin.html`, `event-detail.html`, `events.html`, `filters.html`, `index.html`, `users.html`와 TypeScript source/static JS가 있다. | 계약 entry인 `login.html`, `overview.html`, `status.html`를 제공/정리하고 dashboard session, API-backed overview/status, metadata-only rendering을 완성한다. |
| Overview API / Stats | 교체 필요 | `/stats/users`, `/stats/events`는 있으나 MVP 계약의 `/dashboard/overview`가 없다. | overview aggregate를 `/dashboard/overview`로 구현한다. |
| Events Dashboard API | 교체 필요 | `/events`, `/events/{event_id}`는 metadata-only에 가깝지만 `/dashboard/events*` path와 `inputs[]`/`event_inputs` detail 계약이 아니다. | `/dashboard/events`, `/dashboard/events/{event_id}` 계약과 `input_results`, `content_unavailable_inputs`, Business Context metadata를 구현한다. |
| PostgreSQL schema / migrations | 부분 | auth, login_id, admin seed, `analysis_events`, `event_detections`, `filter_rules` migration이 있다. | `dashboard_sessions`, `event_inputs`, `idempotency_keys` migrations를 추가하고 `filter_rule_versions`를 MVP 밖으로 정리한다. |
| Default ADMIN / env contract | 교체 필요 | migration fallback은 `1234`지만 `.env.example`은 `PROMPTGUARD_INITIAL_ADMIN_PASSWORD=Admin1234!ChangeMe`를 유지한다. | fresh install default를 `admin / 1234`로 둘지 env override로 둘지 제품 결정을 맞추고 `.env.example`/운영 문구를 정리한다. 평문 저장은 금지한다. |
| Extension | 부분 | extension README와 source는 DOM preflight, prompt/file input/drop handling, text file memory-only, fail-closed, mask replace, safe storage/error 경계를 가진다. | 최종 `/prompts/analyze` `inputs[]` contract와 request builder를 맞추고 unsupported attachment metadata/content-unavailable 처리를 완성한다. |
| Privacy smoke | 부분 | API/extension 단위 테스트가 raw prompt, masked prompt, token/secret, filename/file content 일부 금지 경로를 검증한다. | DB/API/DOM/error path를 포괄하는 basic privacy smoke를 명확히 묶고, full privacy/security regression은 post-MVP로 둔다. |
| Final smoke | 미구현 | 통합 MVP smoke 완료 근거가 없다. | extension + API + dashboard session + metadata UI 전체 smoke를 수행한다. |
...
|---|---|---|---|---|---|---|---|---|---|
| 59 Users 관리 화면 구현 | 김현성 | MVP 대시보드 | Users 화면 | mock 데이터를 제거하고 `/dashboard/users` 목록/생성/role/status API를 연결한다. | `users.html`, Users TypeScript, `/dashboard/users` API client 연동 | 부분 | 교체 | 사용자 목록, 사용자 생성, role 변경, status 변경, 비활성화가 실제 API로 동작한다. `/admin/users` 기준 연동은 남기지 않는다. | 김현성. 현재 계약은 `/dashboard/users` 기준이다. |
| 80 사용자별 이벤트 통계 API 구현 | 김현성 | MVP API·데이터·Analyze | Users 통계 API | 기본 통계 기간 내 사용자별 event/action aggregate를 `/dashboard/users` 응답에 포함한다. | `/dashboard/users` response row의 `event_count`, `blocked_count`, `masked_count`, `warned_count`, `last_event_at` | 부분 | 수정 | Users 화면의 사용자별 이벤트 표가 별도 mock 없이 `/dashboard/users` 응답만으로 렌더링된다. 기존 `/stats/users`, `user_id/display_name` 기준은 `/dashboard/users`, `login_id/username` 기준으로 교체한다. | PR #43 author 기준 유지수가 `GET /stats/users`를 구현했으나 현재 계약은 `/dashboard/users` 응답 내 aggregate다. WBS 배분은 김현성. |
| 61 서버 상태 대시보드 구현 | 유지수 | MVP 대시보드 | Status 화면 | `status.html`을 `/dashboard/status`와 연결하고 typecheck/build를 통과시킨 뒤 변경분을 커밋·푸시·PR 생성한다. | `status.html`, Status TypeScript, `/dashboard/status` API client 연동, PR | 부분 | 수정 | API/PostgreSQL/Migration/Filter Rules/Last Checked가 표시된다. DB URL, secret, token, stack trace, 필터 설정 상세값, 환경변수 상세값은 표시하지 않는다. | PR #46, PR #48 기준 유지수 작업으로 본다. 병합 후에도 이 문서의 `/dashboard/status` 계약에 맞춰 수정 필요. |
| 74, 76, 77 Dashboard prototype/static pages | 김현성 | MVP 대시보드 | Login 화면 | `login.html`을 dashboard session API와 연결한다. | `login.html`, Login TypeScript, `/dashboard/session/csrf`, `/dashboard/session/login`, `/dashboard/session/me` API client 연동 | 부분 | 수정 | ADMIN credential로 로그인하면 `overview.html`로 이동한다. USER credential은 dashboard session을 만들지 못한다. session 만료 시 login 화면으로 이동한다. | PR #23/#26/#38 author 기준 김영은이다. PR #23/#26/#38 모두 WBS 74, 76, 77 범위다. |
| 74, 76, 77 Dashboard prototype/static pages | 김영은 | MVP 대시보드 | Overview 화면 | `overview.html`을 `/dashboard/overview`와 연결한다. | `overview.html`, Overview TypeScript, `/dashboard/overview` API client 연동 | 부분 | 수정 | Total Events, Blocked, Masked, Warned, Active Users 카드와 action/period 차트가 실제 API 값으로 렌더링된다. | PR #23/#26/#38 기준 첫 dashboard 구현자는 김영은이다. |
| 78, 79 Dashboard Events metadata MVP | 김영은 | MVP 대시보드 | Events 목록 화면 | `events.html`을 `/dashboard/events`와 연결한다. | `events.html`, Events TypeScript, `/dashboard/events` API client 연동 | 부분 | 수정 | 이벤트 목록에 시간, 사용자, 서비스, action, risk, 대표 탐지 category/type, 탐지 수, 입력 수, content unavailable 수, 상세보기 링크가 표시된다. | PR #16 author 기준 김현성이다. |
| 78, 79 Dashboard Events metadata MVP | 김영은 | MVP 대시보드 | Event Detail 화면 | `event-detail.html?event_id=...`에서 `GET /dashboard/events/{event_id}`를 호출해 상세 metadata를 렌더링한다. | `event-detail.html`, Event Detail TypeScript, `/dashboard/events/{event_id}` API client 연동 | 부분 | 수정 | detection summary, detections, input results, content unavailable inputs, Business Context metadata가 표시된다. 입력 본문, 파일 내용, 전체 `masked_prompt`, 원본 파일명, 내부 식별자는 표시하지 않는다. | PR #16 author 기준 김현성이다. |
| 87 Filter Rule 관리 화면 구현 | 유지수 | MVP 대시보드 | Filter Rule Management 화면 | `filters.html`을 `/dashboard/filters*` API와 연결한다. | `filters.html`, Filters TypeScript, Filter Rule list/form/dry-run UI | 부분 | 수정 | built-in/custom/context rule 목록, 생성·수정, enable/disable, dry-run이 동작한다. built-in detector는 허용 필드만 수정된다. | PR #30 author 기준 유지수다. |
| 74, 76, 77 Dashboard prototype/static pages | 김현성 | MVP 대시보드 | Dashboard 공통 API client | dashboard 화면들이 ADMIN session, CSRF, safe error banner, loading/empty state를 공통 방식으로 처리한다. | dashboard shared API client, session helper, CSRF helper, error rendering helper | 부분 | 수정 | `login.html`, `overview.html`, `events.html`, `event-detail.html`, `users.html`, `filters.html`, `status.html`에서 공통 client가 재사용된다. session id는 `localStorage`에 저장하지 않는다. | PR #23/#26/#38 기준 첫 dashboard 구현자는 김영은이다. |
|  | 김현성 | MVP 확장앱 | Options auth/config | Extension options에서 PromptGuard API URL 저장, login/logout, `/auth/me`, `/config/extension` 확인을 구현한다. | options UI, service worker auth client, config cache, auth status UI | 부분 | 수정 | PromptGuard API URL 저장 후 인증 상태와 원격 확장앱 설정을 확인할 수 있다. MV3 service worker inactive 상태를 인증 만료로 표시하지 않는다. | 김현성. |
| 95 DOM selector rerender regression | 김현성 | MVP 확장앱 | Content script selector/hook | 지원 대상 AI 서비스 화면에서 composer, send button, attachment chip 후보를 감지하고 click/Enter 전송을 분석 완료 전 보류한다. | content script selector, send interceptor, replay guard, selector tests | 부분 | 수정 | `@` mention, IME composition, Shift+Enter, picker 동작을 전송으로 오판하지 않는다. protected replay는 1회만 수행한다. | 김현성. |
|  | 김현성 | MVP 확장앱 | Analyze request builder | composer text, converted paste, 작은 text file, attachment metadata, unsupported attachment를 8.1의 `inputs[]` 계약으로 request body에 담는다. | request builder, input normalization, input limit preflight, API client tests | 교체 필요 | 교체 | top-level `prompt`, `input`, `file`, `attachments` 없이 `inputs[]` 하나로 `/prompts/analyze`를 호출한다. 한도 초과 본문은 포함하지 않고 content unavailable metadata로 보낸다. | 김현성. |
|  | 김현성 | MVP 확장앱 | File/attachment capture | raw `File` 객체를 확보한 작은 text file은 읽어서 분석 요청에 포함하고, 그 외 첨부는 metadata-only 또는 `unsupported_attachment`로 표현한다. | file input/drop/paste handler, MIME/extension check, attachment chip metadata capture | 부분 | 수정 | 허용 text file은 `kind: "text"`, `source: "file"`, `content_included: true`로 전송된다. PDF/Office/OCR/archive/binary/image scan은 수행하지 않는다. | 김현성. |
...
|  | 김현성 | MVP 확장앱 | Extension storage/privacy | 확장앱 저장소에는 API URL, token, config cache, sync time, selector metadata만 장기 저장하고 입력 본문 계열 값은 저장하지 않는다. | extension storage adapter, cleanup path, privacy tests | 부분 | 수정 | composer 원문, paste 원문, file text 원문, 전체 `masked_prompt`, 탐지값 원문, 원본 파일명, full request/response body가 장기 저장되지 않는다. | 현재 main에는 extension storage/privacy test와 audit event raw-free 경로가 일부 있다. 최종 `inputs[]` API 연동 이후 장기 저장 금지 경로를 다시 확인한다. |
| 47 로그인·refresh·auth/me API 구현 | 유지수 | MVP 인증·세션·권한 | Auth API | 확장앱 bearer token 흐름을 `/auth/login`, `/auth/refresh`, `/auth/logout`, `/auth/me` 기준으로 구현한다. | FastAPI auth routes, token hash 저장, refresh rotation, auth tests | 부분 | 수정 | access token/refresh token 흐름이 동작하고, refresh token 원문은 저장하지 않는다. `DISABLED` 사용자는 차단된다. | 기존 WBS는 부분 상태다. 현재 문서의 `login_id`, token hash, disabled 차단 계약과 대조한다. |
|  | 김현성 | MVP 인증·세션·권한 | Dashboard Session API | ADMIN dashboard session을 `/dashboard/session/csrf`, `/dashboard/session/login`, `/dashboard/session/logout`, `/dashboard/session/me` 기준으로 구현한다. | dashboard session routes, CSRF, HttpOnly cookie, session hash 저장 | 미구현 | 신규 | ADMIN만 session 생성 가능하고 USER는 403이다. session id는 `localStorage`에 저장하지 않는다. | 김현성 담당. 대시보드 bearer token 사용 방식이 있으면 교체한다. |
| 22 관리자 기반 사용자 생성 API 구현 / 23 사용자 role/status 변경 API 구현 | 김현성 | MVP API·데이터·Analyze | Users API | ADMIN 사용자 관리 API를 `/dashboard/users`, `POST /dashboard/users`, `PATCH /dashboard/users/{login_id}/role`, `PATCH /dashboard/users/{login_id}/status` 기준으로 구현한다. | users API routes, schemas, service, tests | 부분 | 수정 | `login_id` 기준 목록/생성/role/status 변경이 동작한다. email 필수 요구와 `/admin/users` path는 남기지 않는다. | 현재 main에 `/dashboard/users` 목록/생성/role/status patch와 metadata-only response가 있다. 다만 event aggregate는 stub/default이며 dashboard session + CSRF auth boundary가 아니다. |
| 25 Analyze API 요청 schema 검증 / 57 Analyze API 전체 흐름 통합 | 김현성 | MVP API·데이터·Analyze | Analyze API | `POST /prompts/analyze`를 `inputs[]` 기반 send attempt decision endpoint로 교체한다. | analyze request/response schemas, route, pipeline orchestration, tests | 교체 필요 | 교체 | top-level `prompt`, `input`, `file`, `attachments` 없이 `inputs[]`만 수용하고 top-level `action` 하나를 반환한다. | 기존 WBS 25는 prompt/context 중심 schema라 새 `inputs[]` 계약에 맞게 교체한다. WBS 57은 전체 pipeline 통합 범위다. |
| 27 client_request_id 중복 요청 처리 | 김현성 | MVP API·데이터·Analyze | Analyze Idempotency | `(login_id, client_request_id)` 기준 중복 event 방지를 구현한다. | `idempotency_keys` 저장, idempotency service, duplicate request tests | 미구현 | 수정 | 같은 전송 시도의 중복 요청이 두 번째 event를 만들지 않는다. HMAC fingerprint는 구현하지 않는다. | 기존 WBS의 동일 fingerprint 재요청 처리는 MVP에서 제외하고 `client_request_id` 중복 event 방지만 남긴다. |
|  | 김현성 | MVP API·데이터·Analyze | File/Text Input | 작은 text file scan과 attachment metadata/unsupported attachment 표현을 구현한다. | file text read path, MIME/extension allowlist, input limit handling, tests | 부분 | 수정 | 허용 text file은 `kind: "text"`, `source: "file"`, `content_included: true`로 분석된다. 미지원 첨부는 metadata-only 또는 `unsupported_attachment`로 처리된다. | 김현성. PDF/Office/OCR/archive/binary/image scan은 구현하지 않는다. |
| 6 analysis_events·event_detections 테이블 작성 / 7 원문 없는 이벤트 저장 서비스 구현 | 김현성 | MVP API·데이터·Analyze | Events API | 대시보드 이벤트 조회 API를 `/dashboard/events`, `/dashboard/events/{event_id}` 기준으로 구현한다. | events routes, list/detail schemas, event metadata queries, tests | 교체 필요 | 교체 | 목록/상세는 metadata만 반환하고 내부 식별자와 입력 본문을 반환하지 않는다. | 김현성이 넘겨받는다. 기존 WBS에는 `prompt_hash_prefix` 저장/표시가 포함되어 있어 현재 계약에 맞게 제거한다. |
| 10 Overview summary API 연결 | 김영은 | MVP API·데이터·Analyze | Overview API | Overview aggregate를 `/dashboard/overview` 기준으로 구현한다. | overview route, 30일 기본 aggregate, action/period counts, tests | 교체 필요 | 교체 | 카드와 action/period 차트에 필요한 metadata가 반환된다. 사용자별 aggregate는 `/dashboard/users`가 담당한다. | 기존 WBS는 이벤트/사용자/기간 통계를 함께 연결하는 내용이라, 사용자별 aggregate는 `/dashboard/users`로 분리한다. |
| 8 Filter Rule dry-run API 구현 / 29 custom keyword·regex·context_rule 생성·수정 API 구현 / 30 통합 Filter Rule 목록·상세 API 구현 | 김현성 | MVP API·데이터·Analyze | Filter Rule API | 통합 Filter Rule API를 `/dashboard/filters*` 기준으로 구현한다. | filter routes, schemas, `origin/kind`, `editable_fields`, dry-run | 부분 | 수정 | built-in detector는 `enabled`, `severity`, `action`만 수정 가능하고, custom keyword/regex/context rule과 dry-run이 동작한다. | 현재 main은 `origin/kind/label` schema, `/dashboard/filters*`, enable/disable, built-in field restriction, dry-run 방향이 대부분 맞다. 다만 `filter_rule_versions`는 MVP completion 근거가 아니며 dashboard session + CSRF auth boundary가 아니다. |
| 51 서버 health/readiness endpoint 구현 | 유지수 | MVP API·데이터·Analyze | Dashboard Status API | 서버 상태 조회 API를 `/dashboard/status` 기준으로 구현한다. | status route, required/optional status metadata, tests | 교체 필요 | 교체 | API/PostgreSQL/Migration/Filter Rules/Last Checked를 반환한다. secret, DB 연결 문자열, stack trace는 반환하지 않는다. | 현재 main은 `/status/server`와 nested/top-level `status`, `api`, `postgres`, `migrations` payload다. MVP 계약은 `/dashboard/status`와 flat `api_status`, `postgres_status`, `migration_status`, `filter_rules_status`, `last_checked`다. |
| 6 analysis_events·event_detections 테이블 작성 | 유지수 | MVP PostgreSQL | Event schema | `analysis_events`, `event_inputs`, `event_detections` 중심의 event metadata schema를 MVP 계약에 맞게 정리한다. | SQLAlchemy models, Alembic migration, event metadata tests | 부분 | 수정 | 이벤트 metadata가 저장되고 입력 본문, 파일 내용, 전체 `masked_prompt`, 탐지값 원문은 저장되지 않는다. | 유지수 담당으로 유지한다. `prompt_hash_prefix` 관련 schema는 MVP에서 제외하거나 교체한다. |
| 45 users·refresh_tokens 테이블 작성 | 유지수 | MVP PostgreSQL | Auth/User schema | `users`, `refresh_tokens`, `dashboard_sessions` schema를 MVP 인증 계약에 맞게 정리한다. | SQLAlchemy models, Alembic migration, default ADMIN seed | 부분 | 수정 | `login_id`, `username`, `department`, role/status, password_hash, refresh token hash, dashboard session hash가 저장된다. | 유지수 담당으로 유지한다. email 필수 schema는 제거하거나 교체한다. |
| 48 DB migration 실행 골격 작성 | 유지수 | MVP PostgreSQL | Migration 실행 | fresh DB와 restart에서 Alembic migration과 초기 데이터 생성이 안정적으로 실행되게 한다. | Alembic env, migration runner, seed path, Docker smoke | 부분 | 수정 | fresh DB에서 migration 후 기본 ADMIN과 MVP tables가 준비된다. | 유지수 담당으로 유지한다. |
| 58 filter_rules·filter_rule_versions 테이블 작성 | 유지수 | MVP PostgreSQL | Filter Rule schema | `filter_rules` schema를 MVP 통합 Filter Rule 계약에 맞게 정리한다. | `filter_rules` model/migration, origin/kind/editable_fields/config_json columns | 부분 | 수정 | built-in detector, custom keyword, custom regex, context rule이 하나의 `filter_rules` 모델로 저장된다. | 유지수 담당으로 유지한다. `filter_rule_versions`는 MVP에서 제외하고 18장 범위로 둔다. |
|  | 김현성 | MVP 수용·검증 | Docker fresh-install smoke | fresh clone/export 기준으로 `.env.example`, Docker Compose, DB migration, 기본 ADMIN seed를 검증한다. | Docker smoke script 또는 수동 smoke checklist, install docs update | 미구현 | 신규 | `docker compose up --build`, `/livez`, `/readyz`, `/healthz`, 기본 ADMIN login이 통과한다. | 김현성. |
|  | 전체 | MVP 수용·검증 | API unit/integration test | Auth, dashboard session, users, analyze, filters, events, overview, status API의 MVP 계약을 테스트한다. | API test suite, fixtures, CI command | 부분 | 수정 | `cd apps/api && pytest` 또는 대응 명령이 MVP API 계약 기준으로 통과한다. | 전체. |
|  | 김현성 | MVP 수용·검증 | Dashboard typecheck/build smoke | dashboard TypeScript typecheck/build와 주요 화면 loading/empty/error state smoke를 검증한다. | `npm run typecheck`, `npm run build`, dashboard smoke checklist | 부분 | 수정 | `login.html`, `overview.html`, `events.html`, `event-detail.html`, `users.html`, `filters.html`, `status.html`가 API-backed 화면으로 렌더링된다. | 김현성. |
|  | 김현성 | MVP 수용·검증 | Extension checks | selector, send hook, action UX, auth refresh, API client fixture를 검증한다. | extension checks script, fixture, manual real-service smoke | 부분 | 수정 | `python apps/extension/tests/run_extension_checks.py all` 또는 대응 명령이 통과하고 silent allow가 발생하지 않는다. | 김현성. |
|  | 전체 | MVP 수용·검증 | Basic privacy smoke | 입력 본문, 파일 내용, 전체 `masked_prompt`, 탐지값 원문, 원본 파일명, dry-run sample이 저장·표시되지 않는지 확인한다. | privacy smoke test 또는 checklist | 부분 | 수정 | DB, dashboard API/DOM, error response에서 금지값이 확인되지 않는다. full privacy regression은 MVP 이후 범위다. | 현재 main에는 API/extension privacy 단위 테스트가 일부 있지만 DB/API/DOM/error path를 묶는 수용 smoke는 아직 완성되지 않았다. |
|  | 전체 | MVP 수용·검증 | 최종 smoke 시나리오 | 15.5의 최종 smoke 시나리오를 실행한다. | final smoke checklist, 결과 기록 | 미구현 | 신규 | fresh install부터 extension analyze, Allow/Warn/Mask/Block, dashboard metadata 확인까지 끊기지 않고 통과한다. | 전체. |
|  | 전체 | MVP 수용·검증 | 문서 정합성 확인 | README/install/admin/privacy/release 문서를 이 문서의 MVP 계약과 맞춘다. | README/install/admin/privacy/release docs | 부분 | 수정 | 주요 문서가 `/dashboard/*`, `inputs[]`, `login_id`, Filter Rule `origin/kind`, MVP 범위 기준과 충돌하지 않는다. | 전체. |
