# PromptGuard 개발 문서 세트 1.0.0 - 팀 통합본
...
|  | 김현성 | MVP 확장앱 | Extension storage/privacy | 확장앱 저장소에는 API URL, token, config cache, sync time, selector metadata만 장기 저장하고 입력 본문 계열 값은 저장하지 않는다. | extension storage adapter, cleanup path, privacy tests | 미구현 | 신규 | composer 원문, paste 원문, file text 원문, 전체 `masked_prompt`, 탐지값 원문, 원본 파일명, full request/response body가 장기 저장되지 않는다. | 김현성. |
...
| 8 Filter Rule dry-run API 구현 / 29 custom keyword·regex·context_rule 생성·수정 API 구현 / 30 통합 Filter Rule 목록·상세 API 구현 | 김현성 | MVP API·데이터·Analyze | Filter Rule API | 통합 Filter Rule API를 `/dashboard/filters*` 기준으로 구현한다. | filter routes, schemas, `origin/kind`, `editable_fields`, dry-run | 교체 필요 | 교체 | built-in detector는 `enabled`, `severity`, `action`만 수정 가능하고, custom keyword/regex/context rule과 dry-run이 동작한다. | 기존 WBS는 `/filters`와 `source/kind` 기준이므로 `/dashboard/filters`와 `origin/kind` 기준으로 교체한다. |
| 51 서버 health/readiness endpoint 구현 | 유지수 | MVP API·데이터·Analyze | Dashboard Status API | 서버 상태 조회 API를 `/dashboard/status` 기준으로 구현한다. | status route, required/optional status metadata, tests | 부분 | 수정 | API/PostgreSQL/Migration/Filter Rules/Last Checked를 반환한다. secret, DB 연결 문자열, stack trace는 반환하지 않는다. | 기존 health/readiness 구현을 유지하되 대시보드용 `/dashboard/status` 계약을 추가·정리한다. |
...
|  | 전체 | MVP 수용·검증 | Basic privacy smoke | 입력 본문, 파일 내용, 전체 `masked_prompt`, 탐지값 원문, 원본 파일명, dry-run sample이 저장·표시되지 않는지 확인한다. | privacy smoke test 또는 checklist | 미구현 | 신규 | DB, dashboard API/DOM, error response에서 금지값이 확인되지 않는다. full privacy regression은 MVP 이후 범위다. | 설계상 원문 저장 경로가 없으므로 최소 smoke만 수행한다. |
